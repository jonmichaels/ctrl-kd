...
copied. The development corpus is personal and is not distributed — this
repo's tests use synthetic fixtures plus Robert J. Sawyer's public WS7
archive (opt-in, `pytest -m sawyer`; see `tests/SAWYER-CORPUS.md`) that
encode the same behaviors; additional private corpora are tested internally
against this engine, outside this repo.
