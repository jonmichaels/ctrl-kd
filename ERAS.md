...
| 7 | Footnote/endnote note TAGS and non-numeric FORMAT TYPES | WS5+ | **Modelled, UNVERIFIED.** See below |
...
## Known limitation: footnote/endnote note tags and format types (divergence 7)

**The facts.** WSFORMAT.TXT documents two fields on a footnote/endnote's own
symmetrical-sequence block: the tag/number word ("if the high order bit of
this word is set... the low 15 bits are the offset of the internal
sequence" containing "the footnote number tag" -- resolved one level
...
annotations, and `Note.number` stays `None` for a tagged note so it is never
renumbered. `emit._annotated_notes` displays a tag when present (every
format) and formats a plain number per `Note.number_format` (0 symbols via
the classical `*`/`†`/`‡`/`§`/`‖`/`¶` cycle -- a documented fallback, not a
confirmed WordStar sequence; 1/2 upper/lower alpha; 3, and anything
undocumented, arabic).

**Why this is unverified.** Zero documents in the Sawyer archive carry a
footnote/endnote tag or a non-numeric `number_format` -- every real note in
the corpus is plain arabic, untagged. There is no WS7 install output and no
archive specimen to check either feature against, so both are implemented
to spec and pinned only by synthetic constructed-byte fixtures
(`tests/test_ctrlkd.py`), never measured against real WordStar behaviour.

**What a future fix looks like:** a real document (or a WS7-generated one,
built by actually assigning a custom footnote mark or a non-arabic
numbering format inside WordStar itself and saving it) to check the tag
offset and the symbol cycle against.
