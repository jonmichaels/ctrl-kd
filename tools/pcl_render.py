...
------------------------------------------------
Catalog source: HP "PCL 5 Comparison Guide" (HP Part No. 5021-0378,
Edition 1, E1097, 10/97) -- the document the PCL 5 Printer Language
Technical Reference Manual itself points to for typeface family values
("refer to Appendix C in the PCL 5 Comparison Guide for typeface
values", TRM ch.8 "Typeface Family Command"). Every ID below is cited to
a specific table in that guide, not recalled from memory. The formula
(Table C-2/C-3, Appendix C, pp. C-5-C-7): typeface family value = vendor
value (AGFA/CG = 4096) + typeface base value, e.g. CG Times = 5 + 4096 =
4101, Univers = 52 + 4096 = 4148. Table A-1 "HP PCL 5 Commands -- Primary
Typeface Family" (p. A-8) lists the exact ID and escape sequence per
typeface; Tables 3-3/3-4 (pp. 3-3/3-4, Scalable Intellifont/TrueType
Typefaces) additionally list which style variants (Med/It/Bld/Bld It)
share that ONE family value -- style and weight are separate PCL fields
(S = style, B = stroke weight), never separate family IDs. This matters:
an earlier pass in this project invented four distinct family IDs
(4148-4151) for "Univers Medium/Bold/Italic/BoldItalic" that do not
exist -- Univers Med/It/Bld/BldIt (and Univers Condensed's four weights)
all share the single ID 4148; per the vendor+base formula 4149/4150/4151
are actually Bodoni/Rockwell/Melior, unrelated typefaces never observed
in any capture. That fabrication has been removed; see
TYPEFACE_FAMILY_SOURCE_NOTE for the full per-ID accounting.

Typeface IDs actually observed (by running parse_pcl_extended over every
.pcl in vault jon_vault/Projects/software/WordStar/ws7-prints/v1/): 3
(bitmap Courier, LJ6DTP only), 4099 (Courier, all captures), 4101 (CG
Times), 4113 (CG Omega), 4140 (Clarendon), 4148 (Univers), 4168 (Antique
Olive), 4197 (Garamond Antiqua), 4362 (Albertus) -- the last five all
appear only in LJ6DTP.pcl, the display-type/DTP-features capture; 4113/
4140/4168 were missing from the table entirely before this pass (real
gaps, not guesses -- LJ6DTP actually selects them). 4116 (Coronet) is a
real, sourced HP ID that is simply never exercised by this corpus --
kept in the table (an unexpected sans/script typeface should degrade to
a named catalog entry, not a silent default), but untested in practice.
The PCL spacing field (P: 0=fixed, 1=proportional) remains the
authoritative fixed/proportional signal; typeface ID picks the family
*within* that (Times/Helvetica) for proportional faces. Unknown
proportional IDs fall back to Times; unknown fixed IDs fall back to
Courier.

WIDTH VS SHAPE, FOR TYPEFACES afm.py HAS NO METRICS FOR: afm.py only
transcribes Adobe's base-14 (Times/Helvetica/Courier) -- there is no
Garamond, Univers, Clarendon, Antique Olive, CG Omega, Albertus, or
Coronet AFM table, and none is likely to ever exist for these HP-only
faces. So for every corrected/added ID here, the WIDTH comes from
whichever base-14 family is the nearest serif/sans/fixed classification
(a documented typographic fact about the real typeface, cited where
checked below) -- it is a deliberate approximation, not a claim that
e.g. Garamond Antiqua and Times share metrics. The DRAWING font (see
FONT_TTF) is, for now, the SAME base-14 substitute (Nimbus Roman/Sans/
Mono), because no better free shape match is installed on this system
(checked: `fc-list` has no Garamond/Univers/Clarendon/Antique
Olive/Optima family here). A URW Garamond or EB Garamond OTF would be a
strictly better glyph-SHAPE match for Garamond Antiqua while still using
Times-Roman AFM widths (the tool's width/shape split makes this a real,
separately-installable upgrade) -- not implemented in this pass since it
needs a new font package, a system-level change outside tools/.
...
TYPEFACE_FAMILY = {
    ...
    4: "Helvetica",     # Helvetica -- Table C-3 (p.C-5) base value 4, standalone/unprefixed;
                        # documented catalog value, not resident on this corpus's LJ6/LJ4-family
                        # bitmap set (Table 3-1, p.3-2 lists only Courier+Line Printer) -- unobserved
    4197: "Times",      # Garamond Antiqua -- Table A-1 (p.A-8): "Garamond Antiqua (s4197T";
                        # Table 3-3/3-4 (pp.3-3/3-4): styles Antiqua(Med)/Kursiv(It)/Halbfett(Bd)/
                        # Kursiv Halb, all ID 4197. CORRECTED from the prior "Times, unverified"
                        # placeholder -- identity is now sourced. Garamond has no AFM table in
                        # afm.py, so Times-Roman widths/shape remain the nearest available
                        # approximation (see docstring "WIDTH VS SHAPE" -- URW/EB Garamond would
                        # be a better SHAPE match if installed; not done here).
    4362: "Times",      # Albertus -- ADDED (was missing entirely). Table A-1 (p.A-8):
                        # "Albertus (s4362T"; Table 3-3/3-4: "Med (semi-bold), Extra Bld", ID 4362.
                        # Albertus is a serif/glyphic display face (Berthold Wolpe) -- Times is the
                        # nearest available serif approximation; observed in LJ6DTP.pcl.
    4113: "Helvetica",  # CG Omega -- ADDED (real gap: observed in LJ6DTP.pcl, was absent).
                        # Table A-1 (p.A-8): "CG Omega (s4113T"; Table 3-3/3-4, ID 4113. CG Omega
                        # is Compugraphic/Agfa's clone of Zapf's Optima, a humanist sans-serif
                        # (independently checked, not HP-sourced) -- Helvetica is the nearest
                        # available sans approximation, though Optima's flared strokes differ from
                        # Helvetica's grotesque ones.
    4140: "Times",      # Clarendon -- ADDED (real gap: observed in LJ6DTP.pcl, was absent).
                        # Table A-1 (p.A-8): "Clarendon (s4140T"; Table 3-3/3-4: "Bld Cnd", ID 4140.
                        # Clarendon is a 19th-c. slab serif -- serif family, Times nearest available
                        # (visually much heavier than Times, but the only serif option here).
    4168: "Helvetica",  # Antique Olive -- ADDED (real gap: observed in LJ6DTP.pcl as the 72pt
                        # title text, was absent). Table A-1 (p.A-8): "Antique Olive (s4168T";
                        # Table 3-3/3-4, ID 4168. Antique Olive (Roger Excoffon, 1962) is a
                        # well-documented French sans-serif -- Helvetica nearest available.
    4116: "Helvetica",  # Coronet -- CORRECTED from "speculative" to sourced: Table A-1 (p.A-8):
                        # "Coronet (s4116T"; Table 3-3/3-4: "Med It", ID 4116. The ID is real, but
                        # Coronet is actually a 1937 Ludlow SCRIPT/cursive face (independently
...
TYPEFACE_FAMILY_SOURCE_NOTE = (
    ...
    "E1097, 10/97 -- Table A-1 p.A-8 for the exact ID+escape-sequence per "
    "typeface, Tables 3-3/3-4 pp.3-3/3-4 for style-variant coverage, Table "
    "C-2/C-3 Appendix C pp.C-5-C-7 for the vendor+base-value derivation): "
    "4101 CG Times, 4099/3 Courier (scalable/bitmap), 4 Helvetica (Table "
    "C-3 base value, unprefixed; documented but not resident in this "
    "corpus's printers per Table 3-1), 4197 Garamond Antiqua (corrected "
    "from an unverified guess), 4362 Albertus (added -- was missing "
    "entirely), 4113 CG Omega (added, real corpus gap), 4140 Clarendon "
    "(added, real corpus gap), 4168 Antique Olive (added, real corpus "
    "gap), 4116 Coronet (corrected from 'speculative' to sourced, but "
    "still unobserved in any capture). REMOVED as fabricated: 4148-4151 "
    "were previously listed as 'Univers Medium/Bold/Italic/BoldItalic' -- "
    "the Comparison Guide shows Univers Med/It/Bld/BldIt (and Univers "
    "Condensed's four weights) all share the SINGLE family value 4148 "
    "(style/weight are the separate PCL Style/Stroke-Weight fields, never "
    "distinct family IDs); by the vendor+base formula 4149/4150/4151 are "
    "actually Bodoni/Rockwell/Melior, unrelated typefaces, never observed "
    "in any capture, and not re-added. 4148 (Univers) itself is kept, "
    "corrected to a single sourced entry, mapped to Helvetica (Univers is "
    "a well-documented sans-serif). "
    "REMOVED as unsupported: typeface ID 1 (previously commented 'classic "
    "bitmap Pica'->Times) -- 'Pica' does not appear anywhere in the "
    "252-page Comparison Guide, and Table C-3's base-value list has no "
    "value 1 at all (it runs 0, 2, 3, 4, ...). Never observed in any "
    "capture either. Removed rather than kept as a labeled guess, since "
    "an ID with no documentary support AND no corpus usage is not worth "
    "tabulating (falls through to the P-field fixed/proportional "
    "default instead). "
    "STILL APPROXIMATE, not 'confirmed', for every non-Times/Helvetica/"
    "Courier typeface above: afm.py has width tables for exactly those "
    "three base-14 families and no others, so the AFM-family (and, "
    "currently, the drawing-font) choice for Garamond Antiqua, Albertus, "
    "CG Omega, Clarendon, Antique Olive, and Coronet is a serif/sans/"
    "fixed classification judgment, not a metrics match -- the ID-to-"
    "typeface-NAME identity is sourced; the typeface-NAME-to-AFM-family "
