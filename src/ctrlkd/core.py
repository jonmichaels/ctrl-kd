...
class Line:
    ...
    lead_48: float = None
    # `.KR` in force AS THIS LINE ENDS -- STATEFUL like `.lh` above, captured
    # the same way (close_line() reads `fmt.get('kerning', True)`, True
    # being WordStar's own default, per the document's own prose "when
    # kerning is on, which is the default"). Register C7: LJ6DTP's page 2
    # types the identical `` `` ``/`''` pair twice, once under `.kr off` and
    # once under `.kr on`, specifically to demonstrate that the pair only
    # reads as a proper curly DOUBLE quote when kerned -- pdf.py's
    # `_lj_substitute` is the consumer.
    kerning: bool = True
    # `.po` IN FORCE ON THIS LINE, in PRINT COLUMNS (same 10-CPI frame as
    # `.lm`/`.rm`/`.pm` -- `_resolve_cols_arg`), or None for "the document's
    # own default" (doc.meta['page']['po_cols']) -- same "absolute here,
    # normalised against the document default once that is known" contract
    # as `lead_48` above. `.po` is STATEFUL exactly like `.lh`/`.oc`/`.lm`
    # (WSFORMAT: a dot command, and every other one in this family re-fires
    # from where it sits onward); the page-geometry reading in
    # `_parse_page_dot` (pre-text-last-wins, register b31 E3 items 2+3 --
    # ONE resolved answer, the document's OPENING value, for the document
    # default/--diagnose) was never wrong, only incomplete, exactly
    # the diagnosis register C24 already made for `.lh`. LJ6DTP.WS sets `.po`
    # FIVE times -- once for its ordinary .7in body margin, once (2.5") for
    # page 4's racing-stripe checkerboard, which measured 2.05in left of
    # gpcl6's real LaserJet output while every consumer here still treated
    ...
    # and pdf.py's Printed renderer used to read ONE document-wide value
    # (`doc.meta['formatting']['sub_super_roll_48']`, register C22/round
    # 17) regardless of a line's own position relative to the `.sr` that
    # set it -- the same disease the b31 E3 sweep found and fixed for
    # `.pl`/`.hm`/`.fm`/`.pn`. a private WS7 specimen (b33 field notes, N10) isolated
    # it: a superscript that is a line's LAST span, with a LATER `.sr` in
    # the same document, rendered at the wrong position because every span
    # in the whole document -- including ones that printed before the
    # `.sr` line was ever reached -- shared that one late-document value.
    #
    # Unlike `lead_48`/`po_cols`, this field is always the RESOLVED
    # absolute value (never back-dated to None for "agrees with the
    # document default"): `.sr` has no competing style/font precedence
    # chain to fall back through, so there is nothing an emitter needs a
    # None sentinel to defer to -- every line's own value is already
    # complete and correct on its own, from the document's very first line
    # (DEFAULT_SR_48 until the first `.sr`) onward. `doc.meta['formatting']
    # ['sub_super_roll_48']` (the document-WIDE last-occurrence snapshot)
    # is untouched by this field and keeps its own existing consumers
    # (Printed RTF's `\up`/`\dn`, --diagnose) -- their own statefulness is
    # a separate, not-yet-scoped item.
    roll_48: float = None
...
class Block:
    ...
    # The style's own declared PALETTE INDEX (WSFORMAT's 16-colour CGA/EGA
    # table, same index space as an inline symmetric type-1 colour change --
    # see `_parse_style_library`'s `entry['colour']`). None when the style
    # never set one (or none governs this block); 0 is an EXPLICIT "Black",
    # kept distinct from None but merged nowhere -- see effective_span_styles,
    # which treats both the same (colour 0 changes nothing under any known
    # driver palette, so tagging it would only add noise, e.g. to RTF/HTML
    # exports of every plain paragraph). Register C5: LJ6DTP's own "Section
    # Heading Font" style declares colour 3 (50% gray under the LJ6DTP
    # palette) -- a real WS7 LaserJet halftone-screens that into the
    # light, patterned look the paper shows, which solid-black bold sans
    # (style_attrs's own bold, correctly merged) never reproduced alone.
    style_colour: int = None
...
def effective_span_styles(span, block, heading_bold: bool = False) -> frozenset:
    ...
    if heading_bold and block.heading:
        styles = styles | {'b'}
    # Register C5: a paragraph style's own declared colour (block.style_colour,
    # e.g. LJ6DTP's "Section Heading Font" at colour 3) is a DEFAULT for every
    # span the style governs -- merged only when the span carries no explicit
    # colourN of its own (an inline type-1 colour change mid-run overrides the
    # style default, never the reverse) and only when it is NOT 0. Colour 0 is
    # explicit "Black" in the style record, but every known driver palette (and
    # RTF's own colour-0-is-automatic convention) already treats 0 identically
    # to "no colour tag at all" -- tagging it would add a colourN class/control
    # word to nearly every plain paragraph in a styled document for zero visual
    # change, which is not what this merge is for.
    if (block.style_colour and
            not any(t.startswith('colour') for t in styles)):
        styles = styles | {'colour%d' % block.style_colour}
    return styles
...
# b33 N9 (Jon's ruling, 2026-08-26, field notes): the typewriter double
# space after a sentence-ending '.', '?', or '!' -- "convert to single
# space after a period ending a sentence" on Modern by default, kept
# verbatim on Printed/Native by default, either forced by the CLI flag.
# The field notes name '.' explicitly and don't rule '?'/'!' in or out;
# absent a narrower ruling this covers all three real sentence enders
# (the classic typing-class rule, and what "single-space convention"
# means to a reader) -- reported at delivery for Jon to narrow if he
# only meant the bare period.
SENTENCE_END_CHARS = frozenset('.?!')

def sentence_spacing_texts(texts: list) -> list:
    """N9 'single' mode: collapse a run of 2+ literal spaces immediately
    after a sentence-ending character down to exactly one space, carrying
    state ACROSS the boundaries between consecutive text pieces -- so it
    is safe to call on a Line's own per-span text (a style change can
    land inside the space run, or right at the sentence-ending
    character) or on any other ordered sequence of text pieces that
    concatenate into one line. A deliberately SIMPLE textual rule, no
    abbreviation detection (Jon's ruling, "no cleverness"): 'e.g.  x'
    collapses exactly like a genuine sentence end, because the document
    doesn't carry the difference and neither does this.

    Returns a NEW list, same length; a piece whose text is unchanged is
    returned as the SAME string object (cheap no-op check for the
    overwhelmingly common case of a piece with no space run at all)."""
    out = []
    after_end = False    # last non-space character seen was . ? or !
    seen_space = False    # already kept ONE space in the run since then
    for t in texts:
        chars = []
        for ch in t:
            if ch == ' ':
                ...
                chars.append(ch)
                after_end = ch in SENTENCE_END_CHARS
                seen_space = False
        nt = ''.join(chars)
        out.append(t if nt == t else nt)
    return out

def resolve_sentence_spacing(value: str, printed: bool) -> bool:
    """The N9 mode-aware default (Jon's ruling): 'auto' -> single on
    Modern, keep on Printed/Native -- keyed on the EFFECTIVE printed-ness
    (a print stream/ruler-line document renders printed even under a
    'modern' mode request), not the raw mode string, so a forced-printed
    document gets the Printed default it actually renders as. 'keep'/
    'single' (the flag) override in either direction regardless of mode.
    Returns the resolved boolean ('single' is True) every emitter's own
    span-to-output step consults."""
    if value == 'auto':
        return not printed
    return value == 'single'

def sentence_spacing_spans(spans: list) -> list:
    """`sentence_spacing_texts` applied to a list of Spans, styles
    preserved -- the common case every emitter's own span-to-output step
    calls. Cross-span joints are handled (state carries across spans in
    the same call), so this is safe to call on a whole Line's `.spans`
    regardless of where a style boundary happens to fall relative to a
    sentence-ending character or its following spaces."""
    texts = sentence_spacing_texts([s.text for s in spans])
    return [sp if t == sp.text else Span(t, sp.styles)
            for sp, t in zip(spans, texts)]
...
class Note:
    ...
    number: int | None = None  # footnote/endnote only, and only when untagged: the
                                ...
                                # a TAGGED footnote/endnote reports its mark via
                                # `tag` instead, exactly like an annotation)
    tag: str | None = None     # annotations ALWAYS; footnote/endnote SOMETIMES (a
                                # user-supplied mark instead of a number -- WSFORMAT.
                                # TXT's high-bit-on-high-bit case, ruling 2026-08-24
                                # item 4: UNTESTED AGAINST A REAL DOCUMENT, no
                                # archive specimen carries one; synthetic-fixture-
    ...
    number_format: int = 0     # conv-flag high nybble: 0 symbols,1 upper,2 lower,3
                                ...
                                # Honoured by emit.py's label formatting (ruling
                                # 2026-08-24 item 5) for footnote/endnote; values
                                # 0/1/2 (symbols/upper/lower) are UNTESTED AGAINST A
                                # REAL DOCUMENT -- no archive specimen uses anything
                                # but 3 (numeric), synthetic-fixture-verified only.
...
class Document:
    ...
    # The doc.fonts INDEX in force at each line's own .h#/.f# definition, or
    # None when that definition carried no font-change block of its own
    # (register C6: LJ6DTP's `.h1` opens with a type-2 Font block -- Antique
    # Olive, proportional -- that a renderer must resolve the same way a
    # body span's own 'fontN' tag does; PRINTED mode fell back to a hardcoded
    # Courier because nothing captured it). Final state, same limitation as
    # headers/footers above -- a document that redefines a running head's
    ...
    # Raw PCL printer-payload bytes from 0x0F user print controls that carry
    # them, in document order -- indexed by a span's own 'pcl<N>' style tag
    # (same mechanism as pix<N> above). Register C2 (LJ6DTP parity): the
    # border on all 8 pages and page 4's checkerboard are both raw PCL a
    # renderer executes rather than draws generically.
    pcl_programs: list = field(default_factory=list)   # list[bytes]
...
def _block_end(data: bytes, i: int) -> int:
    """Offset of the closing 0x1D of the WS5+ symmetric block starting at `i`,
    or -1 when the framing does not close.

    THE one definition of "this is a real symmetric block". A WS5+ sequence is

        1D <count:u16 LE> <cmd> <payload> <count:u16 LE> 1D

    with the leading and trailing counts EQUAL and the closing bracket
    actually there; `count` spans from the cmd byte through the echoed count,
    so it is len(payload) + 4 and can never be below 4. The spec says a bare
    0x1D "should not appear in files"; WordStar itself TRUNCATES a file when
    fooled by one (engineering note 650, "false symmetrical sequences").
    `_symmetric_blocks` walks with this predicate and `detect` counts with it
    -- one notion of validity, not two."""
    if i + 3 > len(data):
        return -1
    jump = int.from_bytes(data[i + 1:i + 3], 'little')
    end = i + 2 + jump
    if (jump >= 4 and end < len(data) and data[end] == 0x1D
            and int.from_bytes(data[end - 2:end], 'little') == jump):
        return end
    return -1


_SYM_SCAN = re.compile(rb'\x1b.|\x1d', re.S)


def _count_symmetric_blocks(data: bytes) -> int:
    """How many WELL-FORMED 0x1D symmetric blocks `data` contains.

    Counting bare 0x1D BYTES instead was the b28 defect (job-493): byte 0x1D
    turns up by chance about once per 256 bytes, so a 2 MB Windows executable
    carries thousands of them -- dosbox-x.exe had 5,990 -- and detect()'s
    "1D blocks are WS5+ machinery regardless of anything else" rule then
    accepted a 21 MB .exe as a WordStar document and rendered it to a 64 MB
    PDF. Random bytes essentially never satisfy `_block_end`'s framing, so the
    structural count of that same file is 0.

    Walks exactly as `_symmetric_blocks` does -- <1B x> is an extended-
    character escape whose second byte is DATA (a 0x1D there starts nothing),
    and a block that closes is consumed whole -- but scans between the
    interesting bytes in C, because this runs on multi-megabyte files."""
    n = 0
    i = 0
    while True:
        m = _SYM_SCAN.search(data, i)
        if m is None:
            return n
        if m.group()[0] == 0x1B:
            i = m.end()
            continue
        end = _block_end(data, m.start())
        if end < 0:
            i = m.start() + 1
            continue
        n += 1
...
def detect(data: bytes) -> dict:
    ...
    # is just part of the drawing -- the file's real trailing ^Z padding
    # run does not start until offset 6324). Truncating at the first
    # occurrence judged that 6.4 KB document on its first 48 bytes ("89%
    # text but no structure") and parse() refused it outright. _bare_eof()
    # finds the true EOF: the start of the trailing padding run when one
    # exists, else the first bare 0x1A (a document can be genuinely
    # truncated right at EOF with no padding at all).
    _eof = _bare_eof(data)
    core = data[:_eof] if _eof != -1 else data
    if not core:
    ...
    # STRUCTURAL, not a byte tally: `core.count(b'\x1d')` counted chance bytes
    # and let 21 MB of Windows executable in (job-493). See
    # _count_symmetric_blocks.
    blocks_1d = _count_symmetric_blocks(core)
    ...
    # A wrapped triple is three bytes with an ARBITRARY middle, so unlike a
    # symmetric block there is no framing left to verify -- the only
    # corroboration available is density. Chance alone puts a <1B ? 1C> every
    # 65536 bytes of random data, which is why 11 triples in 2 MB of
    # dosbox-x.exe (job-493) meant nothing: that is BELOW the noise floor.
    # Real machinery is orders of magnitude denser -- BOX.WS runs ~90 triples
    # in 304 bytes -- so require at least 4x the chance rate (one triple per
    # 16 KB) on top of the standing count of 3. Measured across the archive
    # the two populations do not overlap: every genuine trip-carrying document
    # sits at >= 7x chance, every binary at <= 2x.
    dense_trips = trips >= 3 and trips * 16384 >= len(core)
    # ONE well-formed block is the threshold, not two. The old `>= 2` was a
    # BYTE threshold and a single real block spends two 0x1D bytes (opening
    # bracket and closing bracket), so `>= 2` bytes and `>= 1` block are the
    # same standard for a genuine document -- measured: 51 archive files
    # (PLAYS.DOC, the MAILLIST envelope layouts, the LSRBOX templates, the
    # WS7 TAGS/ files, -MACROS.DOC) carry exactly one block, and reading the
    # old constant as a block count would have dropped every one of them.
    ...
    # wrapped triples get: chance puts an <8D 0A> every 65536 bytes, and
    # dosbox-x.exe's 12 in 2 MB is BELOW that noise floor -- with the 0x1D
    # count made structural, those 12 chance bytes were the whole remaining
    # case for calling a 21 MB Windows executable a WS4 document. A real WS4
    # document soft-wraps every line: one per ~60 bytes, thousands of times
    # the floor asked for here.
    dense_soft = soft >= 3 and soft * 16384 >= len(core)
    if dense_soft or (hi >= max(1, len(core) // 20) and txt >= 70):
        # WS5+ kept soft returns but dropped the bit-7-on-last-letter
        # convention: a wordstar file with many soft returns and near-zero
        # high bits is WS5+. (Symmetric blocks -- footnotes etc., WS5+ only --
...
def _bare_eof(data: bytes, pad_min: int = 8) -> int:
    ...
    Two things can make a bare 0x1A look like EOF when it isn't:
      - wrapped: the middle byte of a <1B x 1C> wrapped extended character
        ...
        <1B 1A 1C>; cutting at that middle byte amputated 86% of the file);
      - in-content: a lone bare 0x1A used as an ordinary control byte deep
        inside real content, with no EOF significance at all (REF/
        ROUNDED.BRD's box-drawing template hits one at offset 48, but the
        file is a genuine 6.4 KB document -- its real trailing padding
        doesn't start until offset 6324).

    So: collect every BARE 0x1A (skipping wrapped-triple middle bytes),
    then look for a run of >= pad_min CONSECUTIVE bare offsets -- genuine
    disk-block padding, never an isolated in-content byte -- and return
    where that run starts. If no such run exists, the file has no padding
    tail at all; fall back to the first bare 0x1A, same as before this
    function grew run-detection -- a document can be genuinely truncated
    right at a real, unpadded EOF marker.

    Returns -1 when there is no bare 0x1A at all."""
    bare = []
    at = -1
    while True:
    ...
    if not bare:
        return -1
    run_start, run_len = bare[0], 1
    for prev, cur in zip(bare, bare[1:]):
        if cur == prev + 1:
            run_len += 1
        else:
            if run_len >= pad_min:
...
DEFAULT_CW_120 = 12.0      # spec: ".CW ... The default is 12 (12/120ths is 10
                           # characters per inch)."
DEFAULT_SR_48 = 3.0        # WSFORMAT.TXT: "[.SR] ... Default is 3." (1/48in
                           # units) -- the same figure pdf.py/emit.py already
                           # hardcode for their own `.sr`-absent fallback;
                           # named here so Line.roll_48's per-line resolution
                           # (register b32-N10) shares the one constant.
...
_PAGE_DOT_RESOLVERS = {'po_cols': _resolve_cols_arg, 'lh_48': _resolve_lh_arg,
                       ...
                       # NOT a line count. Was falling through to the
                       # lines-at-6-LPI default resolver below, which happened
                       # to be harmless only because nothing consumed pc_col
                       # yet (Table 1, register b31-dot-command-sweep: "never
                       # consumed by ANY renderer"). Register b31, E3 item 2.
                       'pc_col': _resolve_cols_arg}  # everything else: lines at 6 LPI
...
def _parse_format_dot(cmd: bytes, state: dict) -> None:
    ...
    resolve once per document (pre-text-last-wins, register b31 E3 items 2+3
    -- the document's OWN opening geometry, because a page is a page), while
    these are STATEFUL and apply from where they appear onward. A document
    ...
    if name == b'OC':                       # centering on/off
        ...
        if v is not None:
            state['kerning'] = v
    elif name == b'PO':                      # page offset (left origin), print columns
        # Also read (pre-text-last-wins only) by `_parse_page_dot` into the page
        # dict, which is the DOCUMENT-LEVEL default -- pdf.py's `_printed_left`,
        # --diagnose. That reading is not wrong; it is incomplete, the SAME gap
        # register C24 named for `.lh`: `.po` is stateful and re-fires from
        # where it sits onward, and LJ6DTP.WS uses exactly that to move its
        # page-4 checkerboard's left origin to 2.5" mid-document. Carried per
...
def _parse_head_foot(cmd: bytes, doc, encoding: str, anchor=None, font_idx=None):
    ...
    `font_idx` (register C6) is the doc.fonts index the caller found on THIS
    physical line's own marks -- a `.h1`/`.f1` argument can open with a type-2
...
def _parse_page_dot(cmd: bytes, page: dict, meta_extra: dict, has_text: bool = False):
    ...
    Mutates `page` for the `_PAGE_DOT_KEYS` commands with a PRE-TEXT-LAST-WINS
    rule (register b31, E3 open items 2+3, ruled 2026-08-25): `has_text` says
    whether any real body text has been committed to the document yet, at the
    point this dot-command line was reached. While it hasn't (`has_text`
    False), each occurrence OVERWRITES the last -- an author's own immediate
    correction (MICKEE.WS: `.hm 0.22"` then `.hm3`, back-to-back, nothing
    between them) resolves to the LAST one, matching what real WS7 prints
    (measured: the header row moves to the SECOND value). Once body text has
    begun (`has_text` True), this function leaves `page` alone entirely for
    every command in `_PAGE_DOT_KEYS` -- a command repeated mid-document is
    NOT a correction, it is the document's own stateful behaviour, which is
    the domain of the per-page checkpoint machinery in pdf.py
    (`_pl_checkpoints`/`_hm_fm_checkpoints`/`_mt_mb_checkpoints`/
    `_pn_checkpoints`, which walk `doc.meta['dot_positions']` directly and
    never needed this dict to see mid-document changes). `page` therefore
    ends up holding exactly ONE resolved answer -- the document's own
    OPENING geometry -- which is what every consumer that has no per-page
    awareness (RTF export, --diagnose/layout JSON, the `_printed_top`/
    `_printed_notes_reserve_pt` document-wide fallbacks) needs, and it is
    the correct block-0 seed for the checkpoint functions too (most of which
    already seeded at WordStar's own hardcoded default instead of this dict,
    specifically because the OLD "first occurrence anywhere in the document"
    reading could retroactively apply a mid-document-only value to pages
    that printed before the command was ever reached -- see
    `_hm_fm_checkpoints`'s docstring in pdf.py. That degenerate case can no
    longer arise once this function itself refuses to look past the first
    body text).

    `meta_extra` (producer findings, footnote/endnote start numbers) keeps
    its own pre-existing first-occurrence-wins behaviour, untouched --
    it is not part of `doc.meta['page']` and outside this ruling's scope.
    The line is ALWAYS also kept verbatim in doc.meta['dot_commands'] by the
    caller, recognised or not -- this function only adds interpretation on
    top, never replaces preservation.
    ...
    if key is not None:
        if has_text:
            # Body text has already begun: this occurrence is the document's
            # own stateful mid-document behaviour, not an opening correction
            # -- leave `page` (the document's ONE resolved opening answer)
            # alone. The per-page checkpoint machinery (pdf.py) reads
...
def _decode_spans(raw: bytes, strip_hibit: bool, encoding: str, active: set,
    ...
    # tab targets, same mechanism again: one span for the tab's own padding
    # run (already sitting in `raw` as `leader * cols`, see _symmetric_blocks),
    # tagged with the block's ABSOLUTE target (content[2:4], in HMI) so a
    # Printed-mode PDF renderer can set the pen there directly instead of
    # trusting the placeholder's own character-count advance. `leader` rides
    # along on the tag too (as its ordinal) so that renderer can tell a
    # dot-leader tab (real ink WS7 prints) from a plain one (WS7 advances the
    ...
    while i < len(raw) or pending or pending_fonts or pending_colours:
        ...
        # pctl again after tab/pix/fonts/colours fire (and so on) until none
        # apply any more -- not just once per outer-loop pass. Two 0x1D-
        # derived spans can now sit ZERO-GAP adjacent (a tab's own padding
        # ending exactly where the next pctl's display text begins -- LJ6DTP's
        # racing-stripe checkerboard, tab-then-pctl repeated with no literal
        # byte between them, is the real corpus case this was found on): a
        # single pass through pctl-then-pix-then-fonts-then-colours-then-tab
        # advanced `i` past the tab and left it sitting exactly on the next
        # pctl's own start, but control had already moved on to the fnref
        # check and the per-byte dispatch below, which consumed ONE stray
        # byte of that pctl's own text as ordinary content before the outer
        # loop finally looped back around and re-checked `pending_pctl` --
        # one byte late every time, so each subsequent adjacent pctl was
        # entered a further byte behind the last, drawing individual letters
        # ('S', 'h', 'a', 'd', ...) out of what should have been fully
        # invisible screen-only display text. Fonts/colours never consume
        # bytes (their own state-change loops don't advance `i`) so they
        # cannot themselves feed this loop, but they still need to fire in
        # the SAME drain pass as pctl/tab/pix, not stranded a whole
        # outer-iteration behind them, if a font/colour change ever lands on
        # the exact offset a tab or pctl also ends on. The ORDER inside the
        # pass matters too -- see the tab queue's own note below for why the
        # byte-consuming tab must drain after them, not before.
        while True:
            advanced = False
            while pending_pctl and pending_pctl[0][0] <= i < len(raw):
                _, hmi, count, pcl_idx = pending_pctl.pop(0)
                flush()
            ...
            while pending_colours and pending_colours[0][0] <= i:
                ...
                if cnum:                     # colour 0 = Black, the default:
                    active.add(f'colour{cnum}')  # no tag, so fontless output
                                             # and every all-black document is
                                             # byte-identical to before
                advanced = True
            # The tab drains LAST, after the state-only font/colour queues,
            # because it CONSUMES bytes and they do not. WordStar's ordinary
            # encoding for "this line is set in face N and starts at stop M"
            # puts the 0x02 font block and the 0x09 tab block back to back, so
            # both marks land on the SAME offset -- the padding's own first
            # byte. Drained before the fonts (as it was when this queue was
            # first added), the tab cut its padding span with the PREVIOUS
            # line's style set still active and the font change then landed
            # after it. That is not merely cosmetic: `pdf._font_lead_pt` sizes
            # a line's leading to 1.2 x the largest PROPORTIONAL font tagged
            # anywhere on it and carries that size forward through blank lines,
            # so an 18pt title bleeding onto a tab-indented 14pt byline added
            # 1.2 x (18 - 14) = 4.8pt to that line AND 4.8pt again to the blank
            # after it -- 9.6pt cumulative, which cost OLDTIMES.WS (236 type-9
            # blocks, the corpus's worst case) a line off page 1. A tab is a
            # HORIZONTAL instruction and must never move a line vertically;
            # see test_a_tab_never_moves_a_line_vertically. Fonts and colours
            # never advance `i`, so putting them first cannot starve this loop.
            while pending_tab and pending_tab[0][0] <= i < len(raw):
                _, hmi, leader, cols = pending_tab.pop(0)
                flush()
...
def _parse_note(cmd: int, content: bytes, offset: int, encoding: str) -> Note:
        ...
        if remainder[i] == 0x1D and i + 3 <= len(remainder):
            ...
            if inner_cmd == cmd:                        # the internal tag sequence
                ...
                if numeric and len(inner_content) >= 5:
                    # WSFORMAT.TXT, ruling 2026-08-24 item 4: this is the SAME
                    # high-bit rule applied one level deeper ("currently only
                    # one level of this recursion is used" -- so there is no
                    # third level to worry about). The inner block has the
                    # identical 5-byte shape as the outer one (line-count,
                    # tag-word, conv-flag): if ITS OWN tag word also has the
                    # high bit set, this note carries a user-supplied MARK
                    # (a "tag") rather than a resolved number -- read at the
                    # same offset (byte 5 on) an annotation's tag is read
                    # from, since that is the one place in this format a
                    # note's display text already lives. If the inner word's
                    # high bit is clear, it is a plain override number --
                    # unit-tested (test_ws7_note_nested_tag_resolves_number_
                    # and_strips_bytes) since before this ruling, and
                    # unchanged here.
                    inner_tag_word = int.from_bytes(inner_content[2:4], 'little')
                    if inner_tag_word & 0x8000:
                        ...
                        # number stays None: a tagged note has no numeric
                        # identity to renumber (never overwrite a MARK).
                    else:
                        number = inner_tag_word
    ...
    if kind == 'annotation' or tag is not None:
        ...
        # a footnote/endnote carrying a resolved TAG (ruling 2026-08-24
        # item 4, above) is in exactly that case, so its conv_flag byte
        # (the OUTER one -- a tagged note's inner block never reassigns
        # conv_flag) reports no format/convert-to noise either.
...
def _symmetric_blocks(data: bytes, encoding: str, raw_out=None):
    ...
    fonts = []
    includes = []
    # LJ6DTP parity C2: a 0x0F user print control's PRINTER PAYLOAD (the raw
    # bytes WSFORMAT.TXT says go "directly to the printer") used to be
    # computed here just to search it for a `%F"NAME"` file reference, then
    # discarded -- the display string survived (the `pctl` mark below), the
    # actual PCL never did. LJ6DTP.WS embeds its page border (all 8 pages)
    # and page 4's checkerboard entirely as raw PCL in this payload; a
    # renderer that wants to draw them needs the bytes themselves, not the
    # screen-only description. Appended here in document order, indexed by
    # the mark (same mechanism as `graphics`/`pix<N>`).
    pcl_programs = []
    header = {}
    shift_runs = []
    shift_open = []
    ...
    while i < len(data):
        ...
        if data[i] == 0x1D and i + 3 <= len(data):
            start = i
            end = _block_end(data, i)
            if end < 0:
                # A 0x1D whose framing does not close is NOT a block -- the
                # count must echo and the bracket must be there (_block_end,
                # the single definition detect() also counts with). Skipping
                # the byte keeps the document.
                i += 1
                continue
            jump = end - i - 2
            ...
            if cmd in NOTE_KINDS:
                ...
                tab_at.add(len(out))
                # The REAL positioning fact -- content[2:4], "absolute tab
                # size in HMIs" -- travels separately, as a MARK (same
                # mechanism as fnref/pctl/colour), never read anywhere before
                # this. It is ABSOLUTE from the left margin, not relative to
                # wherever the pen sits when the tab fires: MEASURED against
                # real WS7 PCL (LJ6DTP.pcl page 5) -- two table rows whose
                # tabs fire at different pen positions (after "Black" in one,
                # after a dot-leader run in the other) both carry the SAME
                # content[2:4] (4680 HMI), and only reading it as absolute-
                # from-margin reproduces both rows' real bar position; reading
                # it as relative-from-pen would put the second bar 772 HMI
                # (0.43in) further right than WS7 actually printed it. The
                # placeholder text (`leader * cols`, from content[0:2]) is
                # unchanged and keeps every non-positioning consumer (text/
                # RTF/HTML, Show Invisibles, tab_at's reflow classifier)
                # working exactly as before -- only a Printed-mode PDF span
                # need ever consult this mark.
                if len(content) >= 5:      # same guard _tab_columns uses for
                                            # its real (non-fallback) path --
                                            # a malformed block has no real
                                            # absolute position to attach
                    abs_hmi = int.from_bytes(content[2:4], 'little')
                    marks.setdefault(len(out), []).append(('tab', abs_hmi, leader, cols))
                out += leader * cols
                ...
                if len(content) >= 3:
                    ...
                    if name:
                        ...
                        # inline in the mark tuple) so _decode_spans can tag
                        # the span with it exactly like a pix placeholder
                        # tags doc.graphics. None when there is no printer
                        # payload at all (a display-only control).
                        pcl_idx = None
                        if printer:
                            pcl_idx = len(pcl_programs)
                            pcl_programs.append(bytes(printer))
                        marks.setdefault(len(out), []).append((
...
def parse_ws(data: bytes, encoding: str = 'cp437') -> Document:
    ...
    if ws5:
        ...
        (data, notes, blobs, tab_at, graphics, colours, fonts,
         includes, driver, shift_runs, marks, header,
         pcl_programs) = _symmetric_blocks(
    ...
    def _new_block():
        return Block('para',
                     ...
                     style_colour=style_fmt.get('style_colour'))
    ...
    _rt_dots = []
    # Register b31 (E3 open items 2+3): has any BODY TEXT been committed yet?
    # Flips True the moment close_line() below appends a Line that actually
    # carries spans -- the exact same "real content, not just a blank"
    # definition the rest of this function already uses for its own
    # position anchors (`len(doc.blocks) + (1 if cur.lines or cur_line.spans
    # else 0)`, `dot_at`). `_parse_page_dot` reads this (plus the still-
    # unclosed `cur_line.spans`, for a dot line that follows text on the
    # SAME physical line's remainder -- doesn't happen in practice since dot
    # lines are always their own physical line, but cheap to be exact) to
    # decide whether a page-geometry command still gets to move the
    # document's own default (before body text: yes, last one standing
    # wins) or must leave it alone (after body text starts: the value is
    # frozen, and any further change belongs solely to the per-page
    # checkpoint machinery in pdf.py -- see MICKEE.WS/HMFM_PROBE in
    # DOT-COMMAND-SEMANTICS.md).
    _rt_body_seen = [False]
    ...
    def close_line():
        ...
        if cur_line.spans:
            ...
            cur_line.roll_48 = fmt.get('sub_super_roll_48', DEFAULT_SR_48)
            cur.lines.append(cur_line)
            _rt_tally[0] += 1
            _rt_last[0] = cur_line
            _rt_content[0] += 1
            _rt_lastc[0] = cur_line
            _rt_body_seen[0] = True
        cur_line = Line()
    ...
    for _rt_idx, (raw, sep, line_marks) in enumerate(physical):
        ...
        if stripped[:1] == b'.' and not pctl_leads:  # dot command line
            ...
            #
            # Register C6: a `.h1`/`.f1` argument can open with its own type-2
            # Font block (LJ6DTP's does -- Antique Olive, proportional), the
            # same mark _decode_spans's font_at reads for body text. It
            # contributes no bytes of its own, so this line's OWN line_marks
            # (already resolved above, unmutated by anything past the
            ...
            _parse_page_dot(cmd, page, meta_extra,
                            has_text=_rt_body_seen[0] or bool(cur_line.spans))
            # LJ6DTP parity C1: a dot-command line is CONSUMED here -- it never
            # reaches `_decode_spans`, so any mark that landed on it dies with
            # it. That silently dropped LJ6DTP's colour RESTORE: the document
            # sets colour 15 (White) for its knockout bar, then restores colour
            # 0 (Black) immediately after, and that restore's own offset falls
            # on the `.sr 3/48"` line that follows -- so every paragraph after
            # it kept painting white on white and two full paragraphs of page 6
            # vanished. The document itself warns about exactly this hazard in
            # its prose ("otherwise the rest of your text will be invisible --
            # white letters on a white background"), and real WS7 prints them
            # black, so the restore IS in the file and we were losing it.
            #
            # Only FORWARD-LOOKING STATE carries: 'colour' and 'font' are modal
            # and belong to whatever text comes next. 'fnref' is a BACKWARD-
            # looking anchor (see its own placement rule in `lines_pass`) and
            # must never be moved forward; neither must one-shot positional
            # marks. Carried marks re-enter at offset 0 of the next content
    ...
    doc.meta['formatting'] = {
        ...
        if k not in ('centering', 'justify', 'wrap',
                     ...
                     'lead_48',
                     # same reasoning, same shape: meta['page']['po_cols']
                     # (document default) and Line.po_cols (per line).
                     'po_cols')}
    ...
    # Line.po_cols gets the SAME absolute-then-back-dated treatment as
    # Line.lead_48 just above, for the same reason: recorded absolutely
    # (close_line()/blank creation) because the document default was not yet
    # known while parsing, normalised to None here once it is. Register b31.
    default_po = doc.meta['page']['po_cols']
    varying = False
    po_varying = False
    for blk in doc.blocks:
        for ln in blk.lines:
            if ln.lead_48 == default_lead:
                ...
                varying = True
            if ln.po_cols == default_po:
                ln.po_cols = None
            elif ln.po_cols is not None:
                po_varying = True
    ...
    # po_varying is deliberately NOT exported: sr keeps its equivalent
    # internal, and the layout/diagnose schema changes only when both
    # engines move together (b31 gate lesson, 2026-08-25). Nothing in
    # this repo reads it either -- the per-line po_cols field carries
