...
# WordStar's own traditional "symbol" footnote sequence (WSFORMAT.TXT
# conversion-flag high nybble, value 0: "0 means to use symbols") is not
# enumerated anywhere in the format reference beyond that one sentence, and
# no archive document uses anything but numeric (3) -- RESEARCHED, NOT
# ESTABLISHED: this is the classical printer's footnote-symbol cycle
# (asterisk, dagger, double dagger, section mark, parallel, pilcrow), which
# is also Word's own built-in "*, dagger, double-dagger, section-mark"
# symbol note-number-format option -- picked as the documented fallback
# because it is the one
# symbol sequence an actual word processor of WordStar's own era already
# shipped under this exact name, not because it has been confirmed as
# WordStar's. UNVERIFIED AGAINST A REAL WORDSTAR DOCUMENT.
_NOTE_SYMBOLS = ('*', '†', '‡', '§', '‖', '¶')

def _alpha_label(n, upper):
    """1->a, 2->b, ..., 26->z, 27->aa, ... -- bijective base-26, the
    standard way to keep counting past one alphabet's worth of labels."""
    s = ''
    while n > 0:
        n, r = divmod(n - 1, 26)
        s = _ALPHA[r] + s
...
def _note_symbol(n):
    """The nth entry of the symbol cycle (see `_NOTE_SYMBOLS` above),
    doubling once the six-symbol cycle exhausts (7th = '**', 13th = '***',
    ...) exactly as that convention has always extended past six notes."""
    cycle = len(_NOTE_SYMBOLS)
    reps = (n - 1) // cycle + 1
    return _NOTE_SYMBOLS[(n - 1) % cycle] * reps

def _format_note_number(n, number_format):
    """A footnote/endnote's display index, ALWAYS arabic.

    WordStar's format spec documents a conversion-flag high nybble ("0 means to
    use symbols, 1 is for upper case, 2 is for lower case, and three is for
    numbers"), and this function briefly honoured it (ruling 2026-08-24 item 5).
    GROUND TRUTH SAYS OTHERWISE.

    `DISPLAY.WS` -- WordStar's own tutorial file in the archive -- carries a
    footnote at number_format=2 and an endnote at number_format=1. Printed
    through REAL WordStar 7 under DOSBox-X, it puts `1.` and `(1)` on the page:
    plain arabic, the same as every other document. WordStar does not use those
    bits for the printed label. Capture kept as ground truth so this never has
    to be re-derived.

    `number_format` stays PARSED -- preserve-what-you-find governs the IR -- and
    is simply not consulted for display. `n` is arrives 1-based via
    `_display_number`.
    """
    return str(n)
...
def _annotated_notes(doc):
    ...
    lockstep with the trailing note text:
      ...
      footnote/endnote  a user-supplied TAG if the note carries one (ruling
                        2026-08-24 item 4 -- a tag is a MARK, never a
                        number, and never renumbered); otherwise WordStar's
                        real note number for THAT kind — footnote #1 and
                        endnote #1 are different notes with the same
                        number, which is why they need separate counters,
                        not one merged one — formatted per the note's own
                        `number_format` (item 5: arabic/upper/lower/symbol)
    ...
    This is the label every format uses, PAGED formats (Printed, Modern
    PDF) and RTF included — WordStar's own per-page numbers are preserved
    here, untouched. TXT/MD/HTML additionally run `_pageless_notes` (below)
    on top of this, since those three have no pages to keep per-page
    numbers unambiguous.
    """
    ...
    for n in doc.notes:
        ...
        if n.kind == 'annotation':
            ...
            if n.tag is not None:
                ...
                index = _display_number(doc, n, counters[n.kind] - 1)
                label = _format_note_number(index, n.number_format)
        else:
            label = str(counters[n.kind])
        out.append((n, label))
    return out

def _pageless_notes(doc):
    """`_annotated_notes(doc)`, with footnote/endnote labels renumbered
    CONTINUOUSLY across the whole document wherever WordStar's own
    per-page-reset numbers actually COLLIDE (ruling 2026-08-24: "renumber
    ONLY when the stored numbers actually collide"). TXT, MD and HTML have
    no pages -- WS7's per-page footnote reset can flatten two different
    notes onto the same displayed number in a page-less rendering, which is
    invalid HTML (duplicate ids) and ambiguous prose (TXT/MD). Collision is
    the trigger, NOT WordStar's own `.F#`/`.E#` "consecutive throughout the
    document" dot command (REF/WSFORMAT.TXT) -- a document already
    numbering consecutively has no collision and is returned untouched, so
    `.F#` never needs to be parsed at all.

    A TAGGED note (a user MARK, ruling item 4) never participates in
    collision detection and is never renumbered, whether or not its own
    kind collides elsewhere in the document -- a mark has no numeric
    identity to collide or renumber.

    Printed, Modern PDF, and RTF must call `_annotated_notes` directly and
    NEVER this function -- they have real pages (or, for RTF, `\\chftn`
    auto-numbering) and must keep WordStar's own numbers exactly as
    stored.
    """
    pairs = _annotated_notes(doc)
    resolved = {id(n): label for n, label in pairs}
    for kind in ('footnote', 'endnote'):
        members = [(n, l) for n, l in pairs if n.kind == kind and n.tag is None]
        labels = [l for _, l in members]
        if len(set(labels)) == len(labels):
            continue                       # no collision -- untouched
        for i, (n, _) in enumerate(members, 1):
            resolved[id(n)] = _format_note_number(i, n.number_format)
    return [(n, resolved[id(n)]) for n, _ in pairs]
...
def _txt_note_mark(kind, label):
    """TXT's own note-marker wrapper (ruling 2026-08-24 item 3, Jon
    verbatim: '[1] Footnote (1) Endnote. Use the same bracket or
    parenthesis in the body.'): footnotes (and annotations/comments,
    unchanged from before this ruling) keep brackets; endnotes switch to
    parentheses so a footnote and an endnote are distinguishable inline,
    not just in the trailing section. Applies identically to the inline
    body marker and the trailing section entry -- the ruling's own
    wording ("the body") plus its own worked entry format (`(1)`) cover
    both."""
    return f'({label})' if kind == 'endnote' else f'[{label}]'
...
def emit_text(doc, mode='printed', notes=DEFAULT_NOTE_KINDS, toc=False,
    ...
    # Printed is a physical-line facsimile with real `\f` page breaks --
    # WordStar's own per-page footnote numbers stay unambiguous there, same
    # as Printed/Modern PDF (ruling 2026-08-24: "Printed... keep WS7's
    # per-page numbering"). Only Modern TXT is genuinely page-less.
    pairs = _annotated_notes(doc) if printed else _pageless_notes(doc)
    ...
    ss_on = resolve_sentence_spacing(sentence_spacing, printed)
        ...
        for s in line.spans:
            ...
            if note is not None:
                ...
                if note.kind in keep and note.kind != 'comment':
                    seg.append(_txt_note_mark(note.kind, label))
            else:
                seg.append(s.text)
        if ss_on:
    ...
    for kind, title in sections:
        if kind not in keep:
            continue
        items = [(label, sentence_spacing_texts([n.text])[0] if ss_on else n.text)
                for n, label in pairs if n.kind == kind]
        if items:
            text += (f'\n\n{title}:\n'
                     + '\n'.join(f'{_txt_note_mark(kind, label)} {t}'
...
def _md_unit_lines(unit, refs, keep, b, pix_map=None, pictures='off', image_links=None,
    ...
    for line in unit:
        ...
        # join below/above, which adds it itself. Independent of the
        # sentence-spacing flag (even 'keep' must not let an author's own
        # trailing double space at a reflowed line end masquerade as a
        # break the source never asked for).
...
def emit_markdown(doc, mode='printed', notes=DEFAULT_NOTE_KINDS, toc=False,
    ...
    if printed:
        ...
        # A fenced block's contents are already ALL-literal (no CommonMark
        # construct reads inside one), so the N9 MD guard is a non-issue here
        # -- sentence-spacing itself still applies, inherited from emit_text.
        body = emit_text(doc, 'printed', notes=notes, sentence_spacing=sentence_spacing)
        out = '```\n' + body.rstrip('\n') + '\n```\n'
        ...
        if toc:
            ...
            if toc_lines:
                ...
                out += '\n'
        return out
    # Reached only in Modern mode (printed returned above via emit_text's
    # own printed facsimile, `\f` pages included) -- Modern MD has no pages
    # of its own, so it takes the same collision-triggered continuous
    # renumbering as Modern TXT (ruling 2026-08-24).
    pairs = _pageless_notes(doc)
    refs = _ref_pairs(pairs)
    margin = _doc_margin(doc)
    convention_indent, head_position = paragraph_layout_context(doc)
    # N9: always the Modern side of the mode-aware default here (printed
    ...
    #
    # Ruling Q2 (b32 field notes, 2026-08-26): "sr's grouped order wins;
    # ctrl-kd conforms" -- definitions are GROUPED by kind (every
    # footnote, then every endnote, then every annotation, then every
    # comment), not emitted in first-reference document order. Before
    # this, a doc referencing footnote 1, endnote 1, footnote 2 in that
    # order produced defs interleaved the same way ([^1], [^e1], [^2]);
    # sr always grouped ([^1], [^2], [^e1]) and ctrl-kd now matches it.
    # Same kind order as the text/HTML end-matter sections just above
    # (`sections` / `_html_notes_sections`) -- one canonical ordering,
    # not a third invention.
    defs = [f'[^{_md_note_id(n.kind, label)}]: '
            # N9 MD guard: `.strip(' ')` (not just lstrip) -- same reason
...
def _html_line(spans, refs, keep, keep_ws=False, shown_map=None, inline_styling=True,
    ...
    N9 (b33 field notes): the sentence-spacing collapse runs FIRST, on the
    ...
    Planning #204/#206: an earlier round of this function carried a
    `merge_tab_position_tags` flag, passed True only by
    `_html_centered_row`, that stripped `tabhmi<N>`/`tableader<N>` tags
    before coalescing so a spaces-centered row's own tab-run merged into
    the surrounding text instead of being fenced off by it -- worked
    around WSFORMAT.WS's own "4"+tab+"Endnote" row's &nbsp;-run artifact
    by matching sr's then-observed centered-row rendering byte-for-byte.
    Planning #206 found the DEEPER bug: that row (and 5 siblings) should
    never have been classified `centered` at all -- `classify_rows`' own
    spaces-centering heuristic was misreading a real WordStar tab-stop's
    absolute-column jump as hand-typed centering padding. Once
    `has_internal_tab_run` (layout.py) excludes exactly that shape from
    `center_via='spaces'`, no row can simultaneously reach
    `_html_centered_row` AND carry an internal tab-run -- the flag became
    permanently unreachable, so it (and `_html_slice`'s own
    pass-through) were removed rather than left as dead code asserting a
    scenario that can no longer occur. Confirmed byte-for-byte against sr
    after the removal: sr never had an equivalent mechanism at all (its
    own `htmlCenteredRow`'s span-slicing happened to drop the tab tags as
    a side effect of an unrelated, incomplete `sliceSpans` port -- see
    the #206 commit) -- once this row stops being centered on both
    sides, it renders through each engine's ordinary paragraph path,
    which both sides already agreed on before #204 was ever filed."""
    if sentence_spacing:
        spans = sentence_spacing_spans(spans)
    out = []
...
def _html_notes_sections(pairs, keep, linked_kinds=_REF_KINDS, sentence_spacing=False):
    ...
    for kind in ('footnote', 'endnote', 'annotation', 'comment'):
        ...
        for n, label in items:
            ...
            note_text = (sentence_spacing_texts([n.text])[0]
                        if sentence_spacing else n.text)
            text = _html.escape(note_text)
    ...
    for entry in doc.styles:
        ...
        if printed:
            ...
            if entry.get('right_margin_hmi'):
                # `.rm`'s hmi is a COLUMN POSITION, same as `right_margin`
                # everywhere else (register b32) -- convert to columns, then
                # to the actual indent width, before rendering it as inches.
                rm_cols = round(entry['right_margin_hmi'] / 180.0)
                ri_cols = rm_indent_cols(rm_cols)
                if ri_cols:
                    props.append('margin-right:%.2fin' % (ri_cols * 0.1))
...
def _classify_modern_blocks(doc):
    ...
    from .layout import classify_rows, rm_indent_cols, has_internal_tab_run
    ...
    for bi, b in enumerate(doc.blocks):
        ...
        for line in merged_lines(b):
            ...
            internal_tab_run = has_internal_tab_run((sp.text, sp.styles) for sp in line.spans)
            entries.append(('para', lm, cut, b.align, text, internal_tab_run))
            plan.append((bi, line))
...
def _html_slice(line, start, end, refs, keep, shown_map, inline_styling=True,
    ...
    # bridge.
    #
    # N9 scope note: sentence-spacing state does not carry ACROSS a slice
    # boundary (each slice is its own `_html_line` call) -- immaterial for
    # every real structure-row split (bullet/def-list markers, spaces
    # padding) since those land on the marker, never mid-sentence.
...
def _html_centered_row(line, s, refs, keep, shown_map, inline_styling=True,
    ...
    Planning #206 note: this function used to also carry issue #204's
    `merge_tab_position_tags=True` -- a centred structure row's own
    tab-run, merged into the surrounding text purely to match sr's then-
    observed byte shape for WSFORMAT.WS's "4"+tab+"Endnote" row. Planning
    #206 found that row (and 5 siblings) should never classify as
    `centered` at all -- see `layout.has_internal_tab_run`'s own
    docstring. No row can reach this function carrying an internal
    tab-run any more, so that mechanism was removed as unreachable
    (`_html_line`'s own docstring carries the full account)."""
...
def emit_html(doc, mode='printed', title='', notes=DEFAULT_NOTE_KINDS,
    ...
    if styles:
        style_class = {s['slot']: ' class="%s"' % _style_slug(s)
                       for s in doc.styles}
    # HTML has no real pagination in EITHER mode -- a pagebreak is only ever
    # a decorative `<hr class="pb">`, never a page boundary that keeps two
    # same-numbered notes apart (unlike Printed/Modern PDF's real pages).
    # So HTML takes the same collision-triggered continuous renumbering as
    # Modern TXT/MD, in printed mode too (ruling 2026-08-24: "HTML needs it
    # as well" -- measured and ruled without a mode qualifier).
    pairs = _pageless_notes(doc)
    refs = _ref_pairs(pairs)
    parts = []
    printed = mode == 'printed' or _printed(doc)
...
def _rtf_note_dest(note, label, mark_override=None, sentence_spacing=False):
    ...
    Deliberately UNSTARRED (ruling 2026-08-26, register row: "skip the
    backslash * on notes"): the RTF spec's leading `\\*` on a destination
    means "a reader that doesn't recognise this control word should skip
    it entirely," which is the wrong failure mode for note text -- an
    unknown reader would silently drop the footnote/endnote/annotation
    body instead of showing it. Without the star, an unknown reader falls
    back to RTF's generic "unrecognised control word" rule and dumps the
    destination's TEXT inline instead of losing it; a `\\footnote`-aware
    reader (Word, Pages, LibreOffice) is unaffected either way -- it keys
    off the `\\footnote` control word itself, not the destination flag.
    ...
    numbered by WordStar either. `label` is already that mark by the time
    it reaches here (emit.py's `_annotated_notes` resolves tag-over-number),
    so this only needs to route it through the same custom-mark branch
    annotations use rather than reaching for \\chftn. UNTESTED AGAINST A
    REAL DOCUMENT -- no archive specimen carries a footnote/endnote tag.
    """
    flag = r'\ftnalt' if note.kind in ('endnote', 'annotation') else ''
    if note.kind == 'annotation' or note.tag is not None or mark_override is not None:
        # custom mark: the annotation's (or tagged footnote/endnote's) tag,
        # or the `prefixed` scheme's label (e1/a1) standing in for \chftn
        # on any kind
...
def _rtf_style_margins(entry, printed):
    ...
    if printed:
        ...
        ri = 0
        if entry.get('right_margin_hmi'):
            # same b32 fix as `_style_css`'s HTML twin: the hmi is a column
            # POSITION (`.rm`'s own frame), not a width -- convert to
            # columns, then to the real indent, before scaling to twips.
            rm_cols = round(entry['right_margin_hmi'] / 180.0)
            ri = rm_indent_cols(rm_cols) * _RTF_TWIPS_PER_COL
        return li, ri
...
def _rtf_block_lead_48(doc, b, resolved=None):
    ...
    UPDATED (ruling 2026-08-26, register row, b33 field notes N2): this
    used to read ONLY `.lh` dot-state (`Line.lead_48`), which never
    consulted a WS7 paragraph STYLE's own `line_height_vmi` -- a 16pt
    Title/Author style (vmi -2/auto, real leading 1.2x16=19.2pt) got the
    document's flat 12pt body default instead, clipping in Word/TextEdit
    (LYING.WS/WARPRAYR.WS and a private specimen all reproduce this). Now backed by
    `pdf.resolved_printed_leads_48`, which recomputes the SAME per-line
    precedence `_doc_to_pagelines`'s printed branch already uses for the
    PDF page (`.lh` override, else a paragraph style's own vmi-derived
    leading via `_style_lead_pt`/`_entering_lead_pt`, else a WS5+
    font-block's own proportional size via `_font_lead_pt`, else the
    document default) -- collapsed to each block's own FIRST REAL line,
    the ceiling of what RTF's paragraph-only `\\sl` can express (a `.lh`
    change strictly mid-paragraph was already out of scope before this
    fix, and stays so). `resolved`, when given, is that whole-document
    map computed ONCE by the caller (`emit_rtf`'s own printed branch) --
    avoids recomputing it per block; omitted (the lint gate's own direct
    call, and any other caller), it is computed fresh here instead, at
    the cost of doing the whole-document walk again for one block."""
    if resolved is None:
        from .pdf import resolved_printed_leads_48
        resolved = resolved_printed_leads_48(doc)
    if id(b) in resolved:
        return resolved[id(b)]
...
def emit_rtf(doc, mode='printed', notes=DEFAULT_NOTE_KINDS, styles=True,
    ...
    # doc_sa just above, rather than re-walking every block's own leading
    # precedence again for every OTHER block in the document.
    resolved_leads_48 = None
    if printed:
        from .pdf import resolved_printed_leads_48
    ...
    def rtf_seg(spans, b):
        ...
        # N9 (b33 field notes): applied to the RAW incoming spans, before
        # the effective-style merge -- `rtf_seg` is the single choke point
        # every RTF render path (printed physical lines, headings, Modern
        # paragraphs alike) funnels through, so one application here
        # covers all of them.
        if ss_on:
            spans = sentence_spacing_spans(spans)
