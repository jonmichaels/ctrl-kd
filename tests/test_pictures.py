...
@pytest.mark.sawyer
def test_real_corpus_acceptance_all_five_resolve_and_embed(require_sawyer_doc):
    ...
    Tier 2 (sawyer): all 5 labels below are manifest keys
    (tests/SAWYER-CORPUS.md) -- an explicit committed list, not a directory
    sweep."""
    from ctrlkd import pdf
    labels = ['-README.WS (root)', '-SCREEN.WS', 'PREVIEW.WS',
              '-README.WS (APP)', '-README.WS (APP/vDosPlus)']
    paths = {label: require_sawyer_doc(label) for label in labels}
