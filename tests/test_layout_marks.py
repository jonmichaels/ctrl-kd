...
# ------------------------------------------------------- register b32-P1
# Schema addition: an INLINE reference-mark run now carries its own
# 'note_kind', matching the end-matter 'note' item's existing field
# (which has always carried it -- see the module docstring's own 'note'
# entry above) -- closing the one asymmetry in the contract that made a
# consumer look the kind up via `notes[ref]['kind']` instead of reading
# it straight off the mark. Additive only: format stays version 1.

def _footnote_endnote_doc():
    return core.parse_ws(
        _ws_block(0x00, b'') + b'alpha ' +
        _ws_note(0x03, b'A footnote.') + b' beta ' +
        _ws_note(0x04, b'An endnote.') + b' gamma.\r\n')


def test_inline_ref_run_carries_its_own_note_kind():
    from ctrlkd import emit
    doc = _footnote_endnote_doc()
    flow = modern_flow(doc, notes=emit.ALL_NOTE_KINDS)
    ref_runs = [r for it in flow['items'] if it['kind'] == 'para'
                for r in it['runs'] if 'ref' in r]
    assert len(ref_runs) == 2
    for r in ref_runs:
        # the mark's own 'note_kind' must agree with the SAME note looked
        # up the old way, via its 'ref' index into flow['notes'] -- this
        # is a redundant, convenience copy, not a second source of truth.
        assert r['note_kind'] == flow['notes'][r['ref']]['kind']
    assert {r['note_kind'] for r in ref_runs} == {'footnote', 'endnote'}


def test_word_scheme_comment_zero_width_anchor_also_carries_note_kind():
    # the markless comment anchor (round 22) is a 'ref' run too -- P1's
    # addition covers it the same as any other inline reference mark.
    doc = _comment_doc()
    flow = modern_flow(doc, notes=emit.ALL_NOTE_KINDS, note_refs='word')
    para = next(it for it in flow['items'] if it['kind'] == 'para'
                and any('ref' in r for r in it['runs']))
    ref_run = next(r for r in para['runs'] if 'ref' in r)
    assert ref_run['text'] == ''                       # still zero-width
    assert ref_run['note_kind'] == 'comment'
