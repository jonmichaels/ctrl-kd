...
# =================================================== real-corpus sweep
#
# The generic corpus-wide runner over Jon's own private archive (tier 3) is
# relocated out of this public repo entirely -- see the private test suite
# in the companion repo, whose own copy of the `inline_polarity_violations`
# shape runs against this same public engine from outside it.

@pytest.mark.sawyer
def test_rjs_ws_specifically_passes_the_inline_gate(require_sawyer_doc):
    ...
    Tier 2 (sawyer): RJS.WS is one of the ten documents in the committed
    manifest (tests/SAWYER-CORPUS.md) -- see require_sawyer_doc in
    conftest.py."""
    path = require_sawyer_doc('RJS.WS')
