...
def _chance_noise_binary(size=1 << 18, seed=4931):
    """A synthetic executable-shaped blob: about half text-like bytes, half
    high bytes, with 0x1D / <1B ? 1C> / <8D 0A> appearing only at the rate
    chance puts them in random data. No part of any real binary."""
    import random
    rnd = random.Random(seed)
    ascii_pool = bytes(range(0x20, 0x7F))     # usage strings, symbol names
    # machine code and padding: control bytes, plus the high bytes that are
    # STILL control-shaped once detect() masks bit 7 (that masking is why a
    # blob of plain 0x80..0xFF scores 85% text-like and would prove nothing).
    # 0x1A is left out so the blob has no EOF to be truncated at.
    # 0x1B/0x1C/0x8D are left out too: this pool is ~50 values wide, so a
    # random draw would mint wrapped triples and soft returns FAR above the
    # rate real random bytes do. Those two sequences are injected below, at
    # the rate a genuine binary shows them.
    noise_pool = (bytes(b for b in range(0x00, 0x20)
                        if b not in (0x1A, 0x1B, 0x1C))
                  + bytes(b for b in range(0x80, 0xA0) if b != 0x8D) + b'\xff')
    blob = bytearray()
    while len(blob) < size:
        pool = ascii_pool if rnd.random() < 0.45 else noise_pool
        blob.append(rnd.choice(pool))
    # chance rates, measured on the real thing: one 0x1D per 256 bytes, one
    # two-byte sequence per 65536
    for off in range(0, size, 256):
        blob[off] = 0x1D
    for off in range(1000, size, 1 << 16):    # 4 wrapped triples in 256 KB
        blob[off:off + 3] = b'\x1b\x41\x1c'
    for off in range(2000, size, 1 << 16):    # 4 soft returns in 256 KB
        blob[off:off + 2] = b'\x8d\x0a'
    return bytes(blob[:size])


def test_detect_refuses_a_binary_whose_1d_bytes_are_chance():
    """job-493: a Windows executable is not a WordStar document.

    detect() used to count BARE 0x1D BYTES -- `core.count(b'\\x1d')` -- and
    treat two of them as "WS5+ machinery regardless of anything else". Byte
    0x1D turns up by chance about once per 256 bytes, so a 2 MB binary
    carries thousands: dosbox-x.exe reported 5,990 of them, detected as ws5+
    at 45% text, and rendered to a 64 MB PDF. Four executables in the archive
    were accepted this way in b28.

    The evidence has to be STRUCTURAL. A block is 1D <count> <cmd> <payload>
    <count> 1D with the counts matching, which random bytes essentially never
    satisfy -- so the same file counts 0. The two-byte sequences (<1B ? 1C>
    wrapped extended characters, <8D 0A> soft returns) can't be reframed, so
    they carry a density floor instead: chance alone puts one every 65536
    bytes, and evidence at or below the noise floor is not evidence."""
    data = _chance_noise_binary()
    assert data.count(b'\x1d') >= 200          # the byte tally that fooled it
    assert core._count_symmetric_blocks(data) == 0    # not one closes
    det = core.detect(data)
    assert det['symmetric_blocks_1d'] == 0
    # the verdict must come from the structure being absent, NOT from the
    # text floor: this blob clears 40% text-like exactly as the .exe did
    assert det['text_pct'] >= 40
    assert det['variant'] == 'binary'
    # ... and the rule is not merely harder to reach: ONE well-formed block
    # in a document still settles it (51 archive files -- PLAYS.DOC, the
    # MAILLIST envelope layouts, the WS7 TAGS/ files -- carry exactly one)
    doc = b'.op\r\n' + b'Ordinary prose that wraps.' + SOFT + \
          ws7_block(0x0B) + b'More prose.' + HARD + b'\x1a'
    real = core.detect(doc)
    assert real['symmetric_blocks_1d'] == 1
    assert real['variant'] == 'ws5+'


def test_bare_eof_finds_true_padding_run_not_first_stray_byte():
    # core.py:1199 used to truncate at the FIRST 0x1A anywhere in the file --
    # but 0x1A occurs legitimately INSIDE real WordStar content (WSFORMAT.TXT
    # 1Dh: "Symmetrical sequences can contain any character including 1AH"),
    # not only as the trailing ^Z padding WordStar writes to fill a CP/M
    # sector. REF/ROUNDED.BRD (Sawyer WS7 preservation corpus) is a genuine
    # 6.4 KB document whose first 0x1A sits at offset 48 -- an in-content
    # control byte inside a real symmetrical sequence -- while its true
    # trailing padding run does not start until offset 6324. Truncating at
    # the first occurrence judged the file on 48 bytes ("89% text but no
    # structure") and parse() refused it outright.
    #
    # No content byte precedes the true padding run in this fixture -- a
    # bare 0x1A appears once early (in-content), then real prose, then a
    # genuine run of >=8 consecutive 0x1A (real padding).
    early = b'abc\x1adef'
    run = b'\x1a' * 10
    data = early + b'more real content after the stray byte' + run
    assert core._bare_eof(data) == len(early) + len('more real content after the stray byte')
    # a file with NO padding run at all -- just one bare 0x1A -- still
    # treats that single byte as EOF (a document can be genuinely truncated
    # right at a real, unpadded EOF marker; this must not regress).
    unpadded = b'plain content ending right here' + b'\x1a'
    assert core._bare_eof(unpadded) == len(unpadded) - 1
    # a wrapped extended character's middle byte is never mistaken for EOF,
    # padding run or not (ASCIITAB.WS charts every control code, including
    # <1B 1A 1C>)
    wrapped_only = b'\x1b\x1a\x1cmore text with no bare 0x1A at all'
    assert core._bare_eof(wrapped_only) == -1


def test_detect_and_parse_survive_in_content_0x1A_before_true_padding_run():
    # Synthetic reproduction of the ROUNDED.BRD shape (real corpus fixtures
    # are not shipped -- see module docstring): a short WS4 header, ONE
    # in-content 0x1A that is not EOF, then substantial real ws4 prose, then
    # a genuine trailing padding run. Before the fix, detect() truncated at
    # the in-content byte and saw only the 18-byte header -- "72% text but
    # no structure", parse() raised ParseError. After the fix, the whole
    # document is in scope.
    header = b'.pl64\r\n.mt1\r\n' + b'\x00\x00\x0f\x00\x00'
    stray = b'\x1a'
    body = (ws4_text('hello there friendly world this line wraps') + SOFT +
            ws4_text('and continues here across several more words') + SOFT +
            ws4_text('a third wrapped line completes the paragraph') + HARD)
    pad = b'\x1a' * 10
    data = header + stray + body + pad

    det = core.detect(data)
    assert det['variant'] == 'ws4'

    doc = core.parse(data)
    assert doc.roundtrip['era'] == 'ws4'
    lines = [b''.join(sp.text.encode() for sp in ln.spans)
             for ln in doc.iter_lines()]
    # all three real prose lines survived -- none silently dropped with the
    # in-content byte
    assert any(b'friendly world' in ln for ln in lines)
    assert any(b'continues here' in ln for ln in lines)
    assert any(b'completes the paragraph' in ln for ln in lines)


def test_detect_genuine_truncation_at_real_eof_is_unaffected():
    # A document with NO padding run -- just one bare 0x1A right after its
    # real content ends -- must still detect and parse exactly as before
    # this fix: _bare_eof's run-detection must not change behavior when
...
def test_emit_text_sections_labeled_by_kind(four_kind_doc):
    ...
    assert 'Endnotes:\n(1) Endnote text.' in t         # ruling 2026-08-24 item 3
...
def test_emit_markdown_note_defs_are_grouped_by_kind_not_reference_order():
    # Q2 ruling (b32 field notes, 2026-08-26): "sr's grouped order wins;
    # ctrl-kd conforms" -- ALL footnote defs first, then ALL endnote defs,
    # regardless of the order the notes are first REFERENCED inline. This
    # fixture references them out of kind-grouped order (footnote 1,
    # endnote 1, footnote 2) specifically to distinguish "grouped by kind"
    # from "first-reference order" -- the two orderings agree unless a
    # test deliberately interleaves kinds like this.
    from ctrlkd.emit import ALL_NOTE_KINDS
    data = (ws7_block(0x00) +
            b'one ' + ws7_note(0x03, b'First footnote.', number=0) +
            b' two ' + ws7_note(0x04, b'First endnote.', number=0) +
            b' three ' + ws7_note(0x03, b'Second footnote.', number=1) +
            b' four' + HARD)
    doc = core.parse_ws(data)
    md = emit.emit_markdown(doc, mode='modern', notes=ALL_NOTE_KINDS)
    i1 = md.index('[^1]: First footnote.')
    i2 = md.index('[^2]: Second footnote.')
    ie1 = md.index('[^e1]: First endnote.')
    assert i1 < i2 < ie1, 'footnote defs must both precede the endnote def'
...
def test_emit_rtf_footnote_uses_real_chftn_destination(four_kind_doc):
    ...
    # unstarred (ruling 2026-08-26): "skip the backslash * on notes"
...
# ---------------------------------------------------- register b31: per-page .pl

def test_mid_document_pl_after_pa_gets_its_own_page_capacity():
    """`.pl` is stateful exactly like `.mt`/`.mb` (Finding 3) -- real WS7
    (PL_PROBE, dosbox-x, register b31-dot-command-sweep) printed 18 lines
    on a page held to `.pl 20`/`.mt 1`/`.mb 1` (cap 18) and 38 on the page
    after a mid-document `.pl 40` (cap 38) -- the SECOND value, not the
    document's first-occurrence one, governs the page it appears on. This
    reproduces it after an explicit `.pa`, the same structural shape as
    the sibling `_mt_mb_checkpoints` tests above."""
    from ctrlkd.pdf import _doc_to_pagelines, _pl_checkpoints, _pl_at
    data = ('.pl20\r\n.mt1\r\n.mb1\r\n' +
            ''.join(f'Apage line {i}.\r\n' for i in range(1, 19)) +
            '.pa\r\n.pl40\r\n' +
            ''.join(f'Bpage line {i}.\r\n' for i in range(1, 61))).encode()
    doc = core.parse_ws(data)
    assert doc.meta['page']['pl_lines'] == 20.0        # global: first occurrence
    checkpoints = _pl_checkpoints(doc)
    # checkpoints[0] itself is WordStar's own hardcoded default (66) --
    # the SEED for "before any real .pl occurrence" -- immediately
    # superseded by another bi=0 entry once `.pl20` (this document's own
    # opening geometry) is walked; `_pl_at(checkpoints, 0)` resolves the
    # one that actually governs block 0, exactly as the render loop does.
    assert checkpoints[0][1] == 66.0
    assert _pl_at(checkpoints, 0) == 20.0
    assert checkpoints[-1][1] == 40.0
    pages = _doc_to_pagelines(doc, True)
    assert len(pages) == 3
    assert [len(p) for p in pages] == [18, 38, 22]      # 60 Bpage lines at cap 38
    assert pages[1].pl_lines == 40.0 and pages[2].pl_lines == 40.0
    assert pages[0].pl_lines is None                   # untouched: "use the doc global"


def test_mid_document_pl_with_no_forced_break_still_gets_its_own_capacity():
    """The organic-break case -- NO `.pa`, the content simply overflows the
    page -- which is exactly PL_PROBE's own shape (real WS7 was never
    asked for a forced break) and which `_mt_mb_checkpoints`'s two real
    oracles (SCRIPT.WS, LJ6DTP.WS) never happened to exercise: both of
    THEIR mid-document changes sit right after a `.pa`, so the pagination
    loop's fresh-page recompute (gated on `not page`, true only once the
    page has actually emptied) fired correctly by construction. Without a
    `.pa`, the line that overflows the OLD page is also the new page's own
    first line, in the SAME loop iteration -- `not page` is still false at
    that point (the old page hasn't been closed yet when the gate is
    checked), so the recompute silently never re-fired for any organic
    break. `_recompute_geom` closed the gap: a second recompute call sits
    right after the organic close, using the line that triggered it."""
    from ctrlkd.pdf import _doc_to_pagelines
    data = ('.pl20\r\n.mt1\r\n.mb1\r\n' +
            ''.join(f'Apage line {i}.\r\n' for i in range(1, 19)) +
            '\r\n.pl40\r\n' +                            # blank line -> new block,
            ''.join(f'Bpage line {i}.\r\n' for i in range(1, 61)))    # no .pa
    doc = core.parse_ws(data.encode())
    pages = _doc_to_pagelines(doc, True)
    # page 1: the 18 Apage lines (cap 18, .pl 20). page 2: the blank
    # (still under the OLD checkpoint -- it precedes ".pl40" in the
    # source) plus the first 17 Bpage lines (18 total, same old cap).
    # page 3: the organic break lands here, recomputes to cap 38 (.pl 40)
    # for the remaining 38 Bpage lines. page 4: the last 5.
    assert [len(p) for p in pages] == [18, 18, 38, 5]
    assert pages[2].pl_lines == 40.0
    assert pages[0].pl_lines is None and pages[1].pl_lines is None


def test_single_geometry_document_never_touches_pl_checkpoints():
    """Mirrors `test_single_geometry_document_never_touches_mt_mb_checkpoints`
    in EFFECT, not in raw checkpoint-list shape: `.pl50` here sits at the
    document's own true start (block 0), so `_pl_checkpoints` resolves it
    to a SECOND entry superseding the hardcoded-default seed at that same
    block (`_pl_checkpoints`'s docstring) -- but the render loop compares
    against `doc.meta['page']['pl_lines']` (also 50.0, core.py's own
    first-occurrence reading), so no page ever gets its own `pl_lines`
    override: byte-identical to before this fix for every document that
    declares its geometry once, up front, and never repeats it."""
    from ctrlkd.pdf import _pl_checkpoints, _pl_at, _doc_to_pagelines
    data = ('.pl50\r\n' +
            ''.join(f'Body line {i}.\r\n' for i in range(1, 21))).encode()
    doc = core.parse_ws(data)
    checkpoints = _pl_checkpoints(doc)
    assert _pl_at(checkpoints, 0) == 50.0 == doc.meta['page']['pl_lines']
    pages = _doc_to_pagelines(doc, True)
    assert all(getattr(pg, 'pl_lines', None) is None for pg in pages)


# ------------------------------------------------- register b31: per-page .hm/.fm
...
def test_mid_document_hm_fm_repositions_header_and_footer_with_mt_untouched():
    ...
    no longer lets this mid-document occurrence become the document's own
    global reading -- `doc.meta['page']['hm_lines']/['fm_lines']` now
    correctly stay at WordStar's hardcoded default (2.0/2.0), matching
    what `_hm_fm_checkpoints`'s own hardcoded-default seed already gave
    block 0. That RETIRES the degenerate case this test used to pin (core
    .py's global reading disagreeing with the checkpoint global on the
    PRE-change pages): pages 1-2 no longer need their own override at all
    ...
    data = ('.he TITLE\r\n.fo FOOTTXT\r\n' +
            ...
            ''.join(f'Page2 line {i}.\r\n' for i in range(1, 61))).encode()
    doc = core.parse_ws(data)
    assert doc.meta['page']['mt_source'] == 'default'      # .mt NEVER appears
    assert doc.meta['page']['hm_lines'] == 2.0 and doc.meta['page']['fm_lines'] == 2.0
    assert doc.meta['page']['hm_source'] == 'default' and doc.meta['page']['fm_source'] == 'default'
    checkpoints = _hm_fm_checkpoints(doc)
    assert checkpoints[-1][1:] == (6.0, 6.0)
    pages = _doc_to_pagelines(doc, True)
    assert pages[0].hm_lines is None and pages[0].fm_lines is None  # agrees
    assert pages[1].hm_lines is None and pages[1].fm_lines is None  # with the
    # now-correct document default (2.0/2.0): no override needed
    assert pages[2].hm_lines == 6.0 and pages[2].fm_lines == 6.0    # these are
    assert pages[3].hm_lines == 6.0 and pages[3].fm_lines == 6.0    # the pages
    # that actually deviate from the document's true opening geometry
    pdf_bytes = emit_pdf(doc, mode='printed')
    ys_h = [float(m) for m in
...
def test_single_geometry_document_never_touches_hm_fm_checkpoints():
    """A document that declares `.hm`/`.fm` once, up front (or never at
    all), never gets a per-page render-time override -- byte-identical to
    before this fix."""
    from ctrlkd.pdf import _hm_fm_checkpoints, _hm_fm_at, _doc_to_pagelines
    data = ('.hm5\r\n.fm5\r\n' +
            ''.join(f'Body line {i}.\r\n' for i in range(1, 21))).encode()
    doc = core.parse_ws(data)
    checkpoints = _hm_fm_checkpoints(doc)
    page = doc.meta['page']
    assert _hm_fm_at(checkpoints, 0) == (5.0, 5.0) == (page['hm_lines'], page['fm_lines'])
    pages = _doc_to_pagelines(doc, True)
    assert all(getattr(pg, 'hm_lines', None) is None
...
def test_mid_document_pn_reanchors_numbering_from_the_page_it_lands_on():
    """`.pn` is stateful too -- WS4 (2026-08-03) only ever measured a
    SINGLE `.pn` (numbering pages 7, 8, 9), never a second one later in
    the same document. Real WS7 (PN_PROBE, dosbox-x, register b31-dot-
    command-sweep) printed page 1 as "10", page 2 as "11" (a `.pn 10` up
    front, incrementing normally), then page 3 as "500" and page 4 as
    "501" once a mid-document `.pn 500` was reached -- re-anchoring the
    count from the page it lands on, exactly like the first `.pn` does.
    Before this, `pn_start` (core.py's "first occurrence wins") was read
    ONCE, globally: this exact document would have numbered every page
    10, 11, 12, 13, silently ignoring the second `.pn` entirely.

    A blank line separates the two `.pn`-bearing sections -- the same
    block-boundary requirement `_pl_checkpoints`'s own organic-break test
    needs: `doc.meta['dot_positions']` is BLOCK-granular, so `.pn 500`
    needs its own block to be distinguishable from `.pn 10`'s."""
    from ctrlkd.pdf import emit_pdf, _pn_checkpoints
    data = ('.he PAGENO #\r\n.pn 10\r\n' +
            ''.join(f'Numline {i}.\r\n' for i in range(1, 111)) +
            '\r\n.pn 500\r\n' +
            ''.join(f'Numline {i}.\r\n' for i in range(111, 221))).encode()
    doc = core.parse_ws(data)
    ...
    assert nums == [10, 11, 500, 501, 502]


def test_single_pn_document_never_touches_pn_checkpoints():
    """A document that sets `.pn` once, up front (or never at all), never
    gets a per-page override beyond the normal +1-per-page count --
    byte-identical to before this fix."""
    from ctrlkd.pdf import emit_pdf, _pn_checkpoints, _resolve_page_numbers, _doc_to_pagelines
    data = ('.he PAGENO #\r\n.pn 7\r\n' +
            ''.join(f'Body line {i}.\r\n' for i in range(1, 111))).encode()
    doc = core.parse_ws(data)
    checkpoints = _pn_checkpoints(doc)
    pages = _doc_to_pagelines(doc, True)
    nums = _resolve_page_numbers(checkpoints, pages)
    assert nums == [7, 8]                                # matches the WS4 finding
    pdf_bytes = emit_pdf(doc, mode='printed')
    rendered = [int(m) for m in
               re.findall(rb'[\d.]+ [\d.]+ Td \(PAGENO (\d+)\) Tj ET', pdf_bytes)]
    assert rendered == nums


# --------------------------------- register b31: E3 open items 2+3 (2026-08-25)
# `_parse_page_dot` pre-text-last-wins -- a back-to-back correction before any
# body text resolves to the LAST value (MICKEE.WS: `.hm 0.22"` then `.hm3`,
# nothing between them, header row measured moving to the SECOND value); a
# repeat once body text has begun is left to the per-page/per-line stateful
# machinery instead, and no longer retroactively becomes the document's own
# opening default.

def test_pretext_back_to_back_hm_last_wins_mickee_shape():
    """MICKEE.WS's own real shape (Sawyer archive): `.hm 0.22"` immediately
    followed by `.hm3`, both before a single character of body text. Real
    WS7 prints the header at the SECOND value's row (measured, register
    b31-dot-command-sweep) -- `doc.meta['page']['hm_lines']` must resolve
    to 3.0 (the LAST one), not 1.32 (0.22in * 6 LPI, the first)."""
    data = ('.hm 0.22"\r\n.hm3\r\n' +
            ''.join(f'Body line {i}.\r\n' for i in range(1, 11))).encode()
    doc = core.parse_ws(data)
    assert doc.meta['page']['hm_lines'] == 3.0
    assert doc.meta['page']['hm_source'] == 'file'


def test_pretext_back_to_back_pl_last_wins_generalizes_past_hm():
    """The pre-text-last-wins rule is a property of the whole page-geometry
    pass (`_PAGE_DOT_KEYS`), not a special case for `.hm` alone -- pin it
    against `.pl` too, two settings back to back before any text."""
    data = ('.pl30\r\n.pl45\r\n' +
            ''.join(f'Body line {i}.\r\n' for i in range(1, 11))).encode()
    doc = core.parse_ws(data)
    assert doc.meta['page']['pl_lines'] == 45.0
    assert doc.meta['page']['size_source'] == 'file'


def test_mid_document_repeat_no_longer_retroactively_becomes_the_default():
    """The other half of the same fix: a command whose ONLY occurrence
    sits AFTER real body text has begun must NOT retroactively become the
    document's own opening default (the exact bug `_hm_fm_checkpoints`'s
    docstring in pdf.py already worked around for the render path -- this
    pins the fix at the SOURCE, `doc.meta['page']` itself). `.mb` here
    only ever appears once, well after the opening prose -- the document's
    global `mb_lines` must stay at WordStar's own hardcoded default (8.0),
    not the mid-document 2.0."""
    data = ('Opening prose, well before any page-geometry command.\r\n' +
            ''.join(f'More opening prose {i}.\r\n' for i in range(1, 11)) +
            '.mb2\r\n' +
            ''.join(f'Later prose {i}.\r\n' for i in range(1, 11))).encode()
    doc = core.parse_ws(data)
    assert doc.meta['page']['mb_lines'] == 8.0
    assert doc.meta['page']['mb_source'] == 'default'
    # ... while the per-page checkpoint machinery still sees it, exactly as
    # it always did -- this fix narrows `doc.meta['page']`, it does not
    # touch the stateful mid-document mechanism at all.
    from ctrlkd.pdf import _mt_mb_checkpoints
    checkpoints = _mt_mb_checkpoints(doc)
    assert checkpoints[-1][1:] == (3.0, 2.0)
...
def test_albertus_and_marigold_resolve_to_glyphic_substitutes_not_generic():
    """Albertus (Monotype, Berthold Wolpe) and Marigold (Agfa Compugraphic,
    Arthur Baker) are real WordStar-era HP LaserJet resident faces -- the
    Sawyer archive's own printer driver files (LASERJET.PDF, LJ6DTP.PDF,
    HP4.PDF) route WordStar's Aachen/ZapfChancery typestyles to them by
    name (`Aachen\\x00Albertus PC ...\\x00`, `ZapfChancery\\x00Marigold PC
    ...\\x00`), and PREVIEW.WS -- WordStar's own factory demo file --
    literally captions its Aachen-typestyle sample "Albertus". Neither
    name has a typestyle NUMBER of its own in WSFORMAT.TXT's 245-entry
    catalog (verified against the public spec: zero occurrences), so
    `TYPESTYLE_NAMES` can never produce either string -- but any caller
    that DOES pass the literal family (a document's own declared face, an
    app font picker, a future producer) must land on a genuinely
    glyphic/flared or calligraphic substitute, never the target's bare
    generic primary."""
    from ctrlkd.fontmap import rtf_fonts, font_stack, TARGET_FONTS
    for target in ('office', 'mac', 'google', 'linux'):
        # A dedicated TARGET_FONTS entry -- not the generic-primary
        # fallback path -- is the only thing that PROVES this target
        # isn't one incidental office/mac coincidence away from silently
        # matching its own generic (office/mac's script generic already
        # happens to be Monotype Corsiva, the same face zapfchancery's
        # own long-ruled entry chose -- a real answer, not a miss, but
        # not distinguishable from the generic by value alone).
        assert 'albertus' in TARGET_FONTS[target]
        assert 'marigold' in TARGET_FONTS[target]
        primary, falt = rtf_fonts('albertus', 'serif', target)
        assert primary is not None
        assert falt != primary
        primary, falt = rtf_fonts('marigold', 'script', target)
        assert primary is not None
        assert falt != primary
    stack = font_stack('Albertus', 'serif')
    assert stack[0] == 'Albertus'                 # pass-through: verbatim name first
    assert stack[1:-1] and stack[-1] == 'serif'   # real alternates precede the generic
    stack = font_stack('Marigold', 'script')
    assert stack[1:-1] and stack[-1] == 'cursive'


def test_document_declaring_albertus_does_not_fall_through_to_generic():
    """End-to-end: a document whose font block IS Albertus (the real
    situation for a WordStar file authored/printed against one of the
    Sawyer archive's own LaserJet drivers, per the note above -- WSFORMAT's
    typestyle-NUMBER table has no slot to express this, only the printer
    driver does) must not have its RTF/HTML output silently collapse to
    the target's generic serif face. Built the way the archive's own
    documents actually get here: parse a real Aachen-typestyle (#50) font
    block, then apply the SAME substitution the corpus's own driver files
    make (Aachen's typestyle -> the real resident face Albertus) by
    setting `typestyle_name` the way a driver-aware producer would -- this
    is not a parser bug being routed around, it is the documented printer-
    driver-level fact the numeric table cannot itself carry."""
    from ctrlkd.typestyles import TYPESTYLE_NAMES
    aachen_n = next(k for k, v in TYPESTYLE_NAMES.items()
                    if v.lower().startswith('aachen'))
    expected_primary = {'office': 'Colonna MT', 'mac': 'Herculanum',
                        'google': 'Cinzel', 'linux': 'DejaVu Serif'}

    def font(n):
        # 0x8000 proportional + 0x0400 generic_style=serif (index 1 of
        # sans/serif/script/display) -- matches the REAL typestyle-50 font
        # block in PREVIEW.WS itself (generic_style: 'serif').
        return ws7_block(0x02, (180).to_bytes(2, 'little') + (240).to_bytes(2, 'little')
                         + ((n & 0x01FF) | 0x8400).to_bytes(2, 'little') + bytes(6))
    data = (ws7_block(0x00) +
            b'Prose padding for detection, a perfectly ordinary sentence.\r\n' +
            font(aachen_n) + b'Albertus  Bold  Italic' + HARD +
            b'Closing prose line keeps the byte ratio looking like text.\r\n')
    doc = core.parse_ws(data)
    assert doc.fonts[0]['typestyle_name'].lower().startswith('aachen')
    doc.fonts[0]['typestyle_name'] = 'Albertus'   # the driver-level real face
    for target in ('office', 'mac', 'google', 'linux'):
        rtf = emit.emit_rtf(doc, mode='modern', fonts_target=target)
        # The Albertus font block's OWN fonttbl entry carries the real
        # glyphic substitute (not just "somewhere in the RTF" -- Georgia's
        # body-font falt is legitimately Times New Roman regardless, so a
        # bare `not in rtf` check on the generic would be meaningless here).
        assert '{\\f2 %s' % expected_primary[target] in rtf, rtf
    html = emit.emit_html(doc, mode='modern')
    # HTML's font_stack is target-agnostic (FONT_ALTS only): the stack
    # must carry a real glyphic alternate BEFORE the bare CSS generic --
    # not `font-family:'Albertus', serif` with nothing in between.
...
def test_lh_before_the_first_one_is_wordstars_own_default_not_the_files():
    """Register b31 (E3 open items 2+3, ruled 2026-08-25): the document
    default is WordStar's own hardcoded 8/48 here, NOT the file's only
    `.lh`, because that `.lh 16` sits AFTER real body text has already
    begun -- a mid-document occurrence is the domain of the per-line
    stateful mechanism (`Line.lead_48`, unaffected by this change), not
    the document-wide default `_parse_page_dot` resolves. The FIRST prose
    line therefore matches the (correct) 8.0 default and back-dates to
    None; the SECOND line is the one that explicitly deviates.

    (Before this fix, `_parse_page_dot`'s "first occurrence anywhere"
    reading retroactively promoted this single mid-document `.lh 16` to
    the document's own global default, which back-dated the FIRST line
    instead -- backwards, since that line printed before `.lh 16` was
    ever reached.)"""
    data = (ws7_block(0x00) +
            b'Prose padding so the detector reads this as a document, plainly.' + HARD +
            b'.lh 16' + HARD +
    ...
    doc = core.parse_ws(data)
    lines = [ln for b in doc.blocks for ln in b.lines]
    assert doc.meta['page']['lh_48'] == 8.0     # WordStar's own default: no
                                                 # pre-text `.lh` occurrence
    assert [ln.lead_48 for ln in lines] == [None, 16.0]  # 16.0 is stated (it
                                                          # deviates); 8.0 now
                                                          # matches the default
...
def test_leading_tab_indent_measures_in_print_columns_not_the_font():
    """WordStar expands a tab to its stop in 10-CPI PRINT COLUMNS, not the
    run's own font advance -- run it at a 72pt display font's own advance
    instead and a one-column offset becomes a six-inch one, which is exactly
    how the archive's banner document (tabs to 1.39in then 1.4in, to print a
    word twice with a 0.1in shadow) threw its second copy off the right edge
    of the paper.

    UPDATED (the real-tab-position fix): the target is now content[2:4], the
    block's own "absolute tab size in HMIs" (ABSOLUTE from the left margin --
    MEASURED against real WS7 PCL, LJ6DTP.pcl page 5), read directly and
    divided by HMI_PER_POINT (25) -- not core._tab_columns's content[0:2]
    rounded to the nearest 10-CPI column and then re-multiplied by a display
    size. Both blocks here carry 2502 in EITHER field (`(2502).to_bytes(2,
    'little') * 2`), so this fixture cannot by itself distinguish the two
    reads; what changed is which one now governs and that no column-rounding
    step sits between the file's own HMI value and the point position:
    2502/25 = 100.08pt exactly, vs the old round(2502/180)=14 columns x
    12 x 0.6 = 100.8pt -- a 0.72pt difference from the rounding step alone."""
    from ctrlkd.pdf import emit_pdf, _printed_left, HMI_PER_POINT
    helv = _helv_typestyle()
    ...
    assert x == round(left + 2502 / HMI_PER_POINT, 1)   # the tab's own
                                                         # absolute HMI target,
                                                         # not 14 columns x the
                                                         # 72pt font's advance
                                                         # nor a column-rounded
                                                         # approximation of it


def test_dot_leader_tab_repeats_the_glyph_instead_of_stretching_it():
    """A DOT-LEADER tab (the tab-stop fix's other bug): the leader run used
    to be Tz-STRETCHED across the true gap (target_x - x) -- the document's
    own declared column count (content[0:2], `_tab_columns`'s `cols`) drawn
    at whatever horizontal scale closed the distance. MEASURED against real
    WS7 PCL (LJ6DTP.pcl page 5, the "Shading 85%" row): between
    ESC&a2066H and the pattern fill that starts the row's bar at 3168, WS7
    prints 27 literal periods at THEIR OWN NATURAL WIDTH -- no Tz-equivalent
    horizontal-scale escape anywhere in the capture -- while the type-9
    block's own declared run is only 16 characters. WordStar REPEATS the
    leader glyph as many times as fit the gap; it never stretches a fixed
    count onto it.

    Pinned here against the COUNT, not just presence: for a 12pt Helvetica
    span (period natural width = afm.string_width_pt('.', 'Helvetica', 12)),
    a tab landing `gap` points past the pen must draw exactly
    floor(gap / period_width) periods, unscaled (no Tz operator, or one at
    100.0), and the pen must still land EXACTLY on the tab's own absolute
    target afterward -- same as every other tab branch."""
    from ctrlkd.pdf import emit_pdf, _printed_left, HMI_PER_POINT
    from ctrlkd.afm import string_width_pt
    helv = _helv_typestyle()
    target_hmi = 5000                        # comfortably past "Row"'s own end
    tab = ws7_block(0x09, (100).to_bytes(2, 'little') +
                    target_hmi.to_bytes(2, 'little') + bytes([ord('.'), 0]))
    data = (ws7_block(0x00) +
            b'Prose padding so the detector reads this as a document, plainly.' + HARD +
            _font_block(helv, 12.0, width=210, style_bits=0x8000) +
            b'Row' + tab + b'Contents' + HARD +
            b'A closing line of ordinary prose keeps the byte ratio honest.' + HARD)
    doc = core.parse_ws(data)
    left = _printed_left(doc, 12)
    spans = _content_spans(emit_pdf(doc, 'printed'))
    dots = next(s for s in spans if set(s[5]) == {ord('.')})
    contents = next(s for s in spans if s[5] == b'Contents')
    target_x = left + target_hmi / HMI_PER_POINT
    period_w = string_width_pt('.', 'Helvetica', 12)
    expected_count = int((target_x - dots[3]) / period_w)
    assert len(dots[5]) == expected_count == 49    # REPEATED to fill, not a
                                                    # fixed count stretched
                                                    # (the block's own
                                                    # declared column count)
    assert dots[2] == 100.0                        # never Tz-STRETCHED --
                                                    # the only Tz here is the
                                                    # unrelated 100.0 reset
    assert contents[3] == round(target_x, 1)       # pen still lands exactly
                                                    # on the tab's own target


def test_a_tab_never_moves_a_line_vertically():
    """A tab is a HORIZONTAL instruction; it must not touch vertical layout.

    THE DEFECT (OLDTIMES.WS, 236 type-9 blocks, the whole corpus's worst
    case): the real-tab-position fix gave the tab's own padding run its own
    span, drained inside `_decode_spans` BEFORE the font/colour queues at the
    same offset -- so a font change landing exactly where a line's leading tab
    begins (WordStar's ordinary encoding for "this line is set in face N and
    starts at stop M": the 0x02 font block, then the 0x09 tab block, then the
    text) no longer reached that padding. The padding kept the OUTGOING font
    tag instead of the incoming one, and `_font_lead_pt` -- which sizes a
    line's leading to 1.2 x the LARGEST proportional font tagged ANYWHERE on
    it, and carries that size forward through blank lines via `state` --
    duly measured the line against the previous, larger face. OLDTIMES's
    18pt title bled onto its 14pt byline: 1.2 x (18 - 14) = 4.8pt on the
    byline's own line, 4.8pt again on the blank line after it (whose carried
    ...
    from ctrlkd.pdf import emit_pdf
    helv = _helv_typestyle()
    tab = ws7_block(0x09, (10).to_bytes(2, 'little') +
                    (1000).to_bytes(2, 'little') + b' \r')
    ...
    def _ys(lead_in):
        data = (ws7_block(0x00) +
                ...
                b'Title' + HARD +
                _font_block(helv, 14.0, width=200, style_bits=0x8000) +
                lead_in + b'Byline' + HARD +
                _font_block(helv, 14.0, width=200, style_bits=0x8000) +
                b'Third' + HARD +
                b'A closing line of ordinary prose keeps the byte ratio honest.'
                + HARD)
        spans = _content_spans(emit_pdf(core.parse_ws(data), 'printed'))
        return {w: next(s[4] for s in spans if s[5] == w)
                for w in (b'Title', b'Byline', b'Third')}
...
def test_a_font_change_at_a_tabs_own_offset_reaches_the_tabs_padding():
    """The mechanism behind the test above, pinned at the parse layer:
    `_decode_spans` drains its mark queues in a loop, and the BYTE-CONSUMING
    tab queue must fire AFTER the state-only font/colour queues, so a mark
    sitting at the padding's own first offset is in `active` before the
    padding span is built. WordStar writes exactly this adjacency for a
    styled, tab-indented line, and before the fix the padding was cut with
    the previous line's style set still active."""
    helv = _helv_typestyle()
    tab = ws7_block(0x09, (10).to_bytes(2, 'little') +
                    (1000).to_bytes(2, 'little') + b' \r')
    data = (ws7_block(0x00) +
            ...
            + HARD)
    doc = core.parse_ws(data)
    line = next(l for b in doc.blocks for l in b.lines
                if any(s.text.strip() == 'Byline' for s in l.spans))
    pad = next(s for s in line.spans
               if any(t.startswith('tabhmi') for t in s.styles))
    text = next(s for s in line.spans if s.text.strip() == 'Byline')
    pad_font = next(t for t in pad.styles if t.startswith('font'))
    text_font = next(t for t in text.styles if t.startswith('font'))
    assert pad_font == text_font        # the INCOMING face, not the title's
...
def test_modern_pdf_endnotes_collect_at_document_end():
    ...
    roman so footnote 1 and endnote i cannot collide. Entry format (`i.`,
    no brackets) per ruling 2026-08-23/24 item 2."""
    from ctrlkd.pdf import emit_pdf
...
def test_modern_outputs_all_carry_the_footnote_marker_and_text():
    ...
    assert r'\*\footnote' not in r        # unstarred (ruling 2026-08-26)
    pdf = emit_pdf(doc, 'modern')
...
def test_note_refs_prefixed_scheme_matches_markdown_labels():
    ...
    assert b'e1' in texts and b'e1.' in texts         # endnote: inline mark,
                                                       # entry (no brackets --
                                                       # ruling 2026-08-24 item 2)
    assert b'a1' in texts and b'[a1]' in texts        # annotation: unchanged
...
def test_layout_emitter_serializes_the_viewer_contract():
    ...
    d = json.loads(out)
    assert d['format'] == 'ctrl-kd-layout' and d['version'] == 13
