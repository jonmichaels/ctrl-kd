...
def _real_doc():
    return core.parse(open(SCRIPT_WS, 'rb').read())
...
pytestmark_skip = pytest.mark.skipif(
    not os.path.exists(SCRIPT_WS), reason='real WS7 corpus not present on this machine')


@pytestmark_skip
def test_script_ws_html_ladder_preserves_relative_indent():
    doc = _real_doc()
    html = emit.emit_html(doc, mode='modern')
...
@pytestmark_skip
def test_script_ws_rtf_ladder_preserves_character_dialogue_structure():
    doc = _real_doc()
    rtf = emit.emit_rtf(doc, mode='modern')
...
@pytestmark_skip
def test_script_ws_slugline_itself_survives_as_its_own_line():
    doc = _real_doc()
    html = emit.emit_html(doc, mode='modern')
...
@pytestmark_skip
def test_script_ws_detection_is_the_deciding_factor_not_an_accident():
